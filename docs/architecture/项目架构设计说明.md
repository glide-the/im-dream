...
<!-- [同步] 2026-08-25：记录目标匿名 MCP 与规范 OAuth MCP 的真实两轮 Chat、SSE、tool/result、resume、Gateway/账本回执；不把未产生的 Workflow Run 或未登录的 Admin UI 表述为已通过。 -->
...
- Runtime `mcp get` 提供正交 `Status`、`Authentication`、`Transport` 和可选 `Failure-Code`；`mcp list` 仍保持每个 Server 一行，Dream 不通过 token 是否存在或 HTTP 类型猜认证状态。
...
| Runtime 测试 | 99 total，97 passed，2 个显式 official OAuth fixture skipped，0 fail；lint exit 0 |
...
真实验收使用本机正常运行的 Dream、Gateway 和当前真实 PostgreSQL，全部业务步骤走公开生产入口；Admin 使用同一数据库和 `/admin/gateway/requests` 查询路由。测试只调用只读 `auth_status`/`get_server_info`，没有创建、修改或删除远端 Comfy 内容；真实 Thread、Message、Gateway request、账本、transcript 与 timeout 错误记录按协议保留。本轮 MCP 验收走 Chat 链路，没有新增 Dream Workflow Run；因未提供 Admin 后台登录凭据，只完成了同库/查询路由证据核对，没有冒充 Admin UI 目视验收。
...
- 若交付门要求新增 Dream Workflow Run 与 Admin UI 目视证据，需要在不复制 MCP 状态机的前提下另行选择真正消费 MCP 的 Workflow 生产入口，并提供有效 Admin 登录会话；本轮 Chat 证据不能替代这两项。
- 真实 Chat 没有执行 Workspace Read/Bash 或故意杀死 Runtime：真实 workspace 存在/API 与 transcript 已核对，sandbox/Runtime crash 由生产合同 fixture 覆盖，慢 MCP 工具 timeout/output-error 是本轮唯一真实异常反馈。
