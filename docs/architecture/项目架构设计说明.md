...
>
> 发布状态：SDK `0.2.144` 已通过 Trusted Publisher 同时发布到 TestPyPI 与正式 PyPI；Dream 使用正式 PyPI 精确版本和 wheel/sdist SHA-256 锁。Runtime 当前为 `main@eb0b503661449f3eec8c69eddca367d19c2d4e33`，五个 `0.1.1` npm 包均已公开并通过 registry fresh install；发布所用短期 npm token 已撤销。
>
...
| clean-room Runtime package | `@glide-the/ink-claude-code-dream@0.1.1` | npm 公开 selector；对 Dream 输出 `2.1.241 (Claude Code)` 兼容标识；manifest 配对 SDK `0.2.144` |
| Runtime 仓库 | `main@eb0b503661449f3eec8c69eddca367d19c2d4e33` | qualification run `32877673733` 与 publish run `32877869672` 均成功 |
...
| registry 验收 Runtime | selector integrity `sha512-1Pnp0G…ymF59w==`；darwin-arm64 integrity `sha512-jxG4d…Nw+RvA==` | fresh install 只选择当前平台包；两个 CLI alias、manifest 配对与零 `.map` 通过 |
| Runtime executable | darwin-arm64 SHA `a4da19cf…55ad3e2d` | `0.1.1` 公开平台包执行对象 |
| Docker 默认 Runtime | npm `@glide-the/ink-claude-code-dream==0.1.1` | 先装 selector/匹配 Linux 平台包；构建时执行版本、manifest 与 Dream resolver 门 |
...
旧 `0.1.0` qualification/registry 安装树继续作为回滚证据；`0.1.1` 已在独立空目录从公共
registry 安装并通过 selector、两个 alias、manifest 与 no-map 验收；本机默认软链接与 Dream Docker 均固定新版本：
...
```text
@glide-the/ink-claude-code-dream@0.1.1
  └─ selector：根据 process.platform/process.arch 选择可选依赖

@glide-the/ink-claude-code-dream-darwin-arm64@0.1.1
@glide-the/ink-claude-code-dream-darwin-x64@0.1.1
@glide-the/ink-claude-code-dream-linux-arm64@0.1.1
@glide-the/ink-claude-code-dream-linux-x64@0.1.1
```

| 包 | npm registry tgz SHA-256 | registry 大小 |
| --- | --- | ---: |
| selector | `1153970fe79fdaf46e541efa9b5947125db7e9d624e3ad59a2f7bb0706746bff` | 18,408 B |
| darwin-arm64 | `622db6e04089939f1c514923357f9b4dfbb4210fa1504176802bd07e5d737de8` | 27,005,831 B |
| darwin-x64 | `300aced61da86a4c3a880bcccd1eb3c9464387efea49ed5f8bdc8e3167812a18` | 29,157,001 B |
| linux-arm64 | `515d2cb533ce4a1e33532f206a49538ce4f0f37a81ed43a97161927b4b6fef83` | 37,967,888 B |
| linux-x64 | `d4b230f3447c999875955c3dfce34cc49e4904c09c0db1626c96dc5218f6c1d2` | 37,471,572 B |
...
五个包已按四个平台包优先、selector 最后的顺序发布到 npm。公共 packument 均返回 `0.1.1`、MIT 和对应 `os/cpu`；全新目录安装只选择 Darwin ARM64 平台包，`ink-claude-code-dream --version`、`claude --version`、manifest 配对和零 `.map` 检查全部退出 0。表中 SHA-256 由公共 registry tarball 重新下载计算，并与 same-SHA qualification 制品一致。
...
| Runtime 测试 | release CI `111 total, 104 passed, 7 skipped, 0 fail`；lint exit 0 |
...
| npm registry fresh install | `0.1.1` selector + darwin-arm64；两个 CLI alias `--version` 为 `2.1.241 (Claude Code)`；manifest 配对 SDK `0.2.144`；无 `.map` |
