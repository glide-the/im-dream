...
<!-- [同步] 2026-09-17：明确多个 Dream checkout 通过服务端配置消费同一 Claude plugin 共享制品根。 -->
<!-- [同步] 2026-09-17：明确 Dream 应用是 OAuth client、Dream 用户是委托产品主体，Admin operator 仍属于独立管理域。 -->
<!-- [同步] 2026-09-16：MCP 管理持久化改由 Admin Registry134–147 DTO/Service/typed Drizzle 提供，Dream 仅保留协议执行与产品交互。 -->
...
<!-- [同步] 2026-08-25（历史）：MCP Resources 管理面当时迁移到 Dream PostgreSQL 与标准 MCP SDK；数据库持久化已于 2026-09-16 再迁至 Admin Registry134–147。 -->
<!-- [同步] 2026-09-13：当前配对为 SDK 0.2.145 × 已发布 Runtime 0.1.10；按通用产品设计原则同步模块职责、具体操作和验收范围。 -->
<!-- [同步] 2026-09-15：Runtime 0.1.10 恢复 plugin/marketplace 管理，Dream 安装边界复用受检 resolver。 -->

# 项目架构设计说明

<!-- [Sync] 2026-09-16: record complete Dream production database source closure and retain normal cutover gates. -->
<!-- [Sync] 2026-09-16: Dream launch source/Preflight/Run/dispatch/failure persistence now uses Admin DTO/ORM operations including Registry133 replay lookup. -->
<!-- [Sync] 2026-09-16: Dream launch Runtime provisioning consumes Admin Registry130-132. -->
<!-- [Sync] 2026-09-16: public Agent type consumes Admin Registry127-129 and retains local shared-artifact verification. -->
<!-- [Sync] 2026-09-16: confirmation Runtime uses Registry121 claim-bound Admin data ownership. -->
<!-- [Sync] 2026-09-16: all production Agent turns use an Admin owner; ClaudeAgentService contains no database fallback. -->
<!-- [Sync] 2026-09-14: review the Admin sole authentication/data-service target separately from baseline implementation. -->

## 背景与问题

baseline 的 Python Authlib/JWT authority 与生产 PostgreSQL 访问需要迁移至 Admin。完整原文保存于[迁移前历史](history/pre-admin-auth-data-20260914/项目架构设计说明.md)，包括全部功能、接口、流程、版本与测量/回滚方法。

## 目标与边界

[Admin认证与数据消费端设计](admin-auth-data-interaction.md)定义目标和评审范围；Admin唯一维护Request/Response DTO、domain service、typed repository与Drizzle ORM。Dream保留Next页面、FastAPI编排、Agent/EventBus/SSE/共享FS和资源算法，移除全部生产DB连接与用户token签名authority。

Registry130-133 已关闭 Dream launch 的数据库路径：Dream 请求边界把 current OAuth 注入统一 Admin client，以严格 Pydantic DTO 调用 replay lookup、Runtime scope/plan/prepare、source ensure、Preflight execute/read、Run create/read、dispatch claim/finish、Run fail 与 failure envelope；Admin 的 Zod DTO、Service、typed Drizzle Repository 执行身份与实体权限、冻结 replay、锁、事务、receipt 和全部 SQL。Dream 继续校验共享 artifact、运行 Agent turn、发送 EventBus/SSE，并执行共享文件系统操作；launch 模块没有 PostgreSQL import、连接、ORM 或 SQL fallback。

Registry121 关闭 confirmation dispatcher 的 Runtime 数据回退：Dream 用严格 Pydantic `claim-turn` 消费 Admin 在同一事务生成的 claim-bound `server-persistence` grant，并复用既有 Workflow、Deck 与 `AdminTurnPersistence` DTO 客户端。Dream 仍执行同 Thread Runtime、EventBus、SSE 和共享文件操作；Admin 执行消息 claim、主体/实体权限、事务和全部持久化。生产数据库入口清单现已全部关闭；正常数据库 capability 部署与真实业务验收仍按发布门禁执行。

Claude plugin 的共享文件拓扑由 `INK_CLAUDE_PLUGIN_RUNTIME_ROOT` 显式选择绝对服务端根目录。Admin 保存 package/version/digest/状态，Dream 从该根派生路径并重新验证不可变 artifact；多个 checkout/worktree 不得各自默认到空目录，也不把路径移入数据库 DTO。artifact 缺失或摘要不符时，Deck 创建与 Runtime 启动在文件操作边界失败并保留数据库事实。

生产 Agent turn 的数据库边界现已统一：Chat、Reflections、launch、confirmation 和 Guidance 在进入 `ClaudeAgentThreadFactory` 前构造绑定 actor、Thread 与适用 Run 的 Admin Workflow、Deck、`AdminTurnPersistence` owner。`ClaudeAgentService` 只调用严格 Pydantic DTO consumer；Thread/Session/消息、workspace plugin、managed MCP scope 与 Runtime activation 的 SQL、权限过滤和事务由 Admin Zod DTO、Service 与 typed Drizzle Repository 执行。缺 owner、capability、DTO 身份不匹配或 Admin 不可用时在 Runtime 前失败，禁止回退 Dream PostgreSQL。Dream 的 Runner、ThreadFactory、EventBus、SSE、turn/resume/cancel、共享文件系统和 `.claude-tmp` 行为不变。

## 概念与规则

实施状态以[执行计划](../exec/dream-admin-auth-data-plan.md)和[生产数据库入口清单](../exec/dream-admin-data-inventory.md)为准。当前源码边界已经关闭 Dream 生产数据库访问；设计契约、provider-free 测试与隔离数据库回执仍不等于正常数据库 capability 已部署，也不等于 Google、Device Flow、真实模型和完整业务验收已经完成。下文保留的 baseline 实现与阶段回执只用于历史追溯。

> 前端代码证据：`54f3bbe539b086035bce5900618bf3d7ae5b9372`（2026-09-06）。Runtime registry 查询日期：2026-09-13。
>
> 当前状态：Dream 源码使用自有 Python SDK `0.2.145`，默认 Runtime 合同为已发布的原始模块统一实现 `0.1.10` package-root selector；官方 Claude CLI 只作为显式绝对路径回滚。本文不据源码推断正在运行的服务版本。
>
> 发布状态：SDK `0.2.145` 的正式 PyPI 精确版本/wheel/sdist SHA-256 锁不变。Runtime `0.1.10` 已完成同 SHA 四平台资格、五包公开归档 integrity、全新安装及本机 Dream 启动身份核对，具体范围见[发布接入回执](../deploy/runtime-0.1.10-plugin-management-and-dream-adoption.md)。本稿不以制品测试替代真实业务验收。
>
...
flowchart LR
    ...
    U -->|"Dream 登录/设备同意"| AUTH["Admin Better Auth/OAuth<br/>Authorization Server"]
    FE -->|"Code + PKCE public client"| AUTH
    API -->|"client_credentials confidential client"| AUTH
    ...
    API -->|"user bearer + service bearer<br/>strict business DTO APIs"| ADMIN
...
| 组件 | 当前职责 | 禁止越界 |
| --- | --- | --- |
| Dream Web | `frontend/` 根 Next.js package；作为 public OAuth client 发起 code/PKCE，浏览器只持 HttpOnly opaque handle | 不恢复第二 Web 入口；浏览器不持有 OAuth access/refresh token、Provider Key、MCP Token 或数据库凭据 |
...
| Dream API | confidential OAuth client；用户调用转发 user bearer 并附加 service bearer，后台调用只用具名 service scope；继续负责 Thread/Run/Workspace 编排、managed MCP、Agent Runtime、SSE 与共享文件 | 不连接 PostgreSQL、不把 Dream user 当 client、不管理共享 Schema，不实现第二套 Agent/Runtime 协议 |
| Admin | Authorization Server、独立 Admin operator 管理域、业务 DTO/Service/typed Drizzle Repository、Drizzle Schema、PostgreSQL、模型、订阅、计费、Gateway | Admin operator 与 Dream user 不合并；Dream Session/token 不取得 Admin RBAC；Dream 不复制身份、事务、权限过滤、账本和模型路由 |
| 自有 Python SDK | 上游兼容 API、transport、`cli_path` 注入 | 不加入 Dream DTO、数据库或业务状态机 |
| 原始模块 Runtime | JSONL、Provider/tool loop、resume、MCP、Workspace、sandbox | 不包含 IM 业务代码或用户可变数据；当前 0.1.10 同 SHA 发布资格已完成；后续版本仍须新资格 |
...
| 对象 | 当前版本或身份 | 当前适用性 |
| --- | --- | --- |
| Dream 项目元数据 | backend `0.1.4`、frontend `0.0.4`；API schema `2.0.0` 不变 | 现有项目版本序列的 patch 更新，不代表部署或 API 变更 |
...
| 原始模块 Runtime package | source contract `@glide-the/ink-claude-code-dream@0.1.10` | `package/cli.js` selector；对 Dream 输出 `2.1.241 (Claude Code)`；manifest 配对 SDK `0.2.145`；同 SHA 四平台资格、五包发布与本机采用已验收 |
...
| registry 验收 Runtime（历史） | 0.1.4 source tree `266362ac…95de81`；darwin-arm64 executable `969f9193…980dd` | 只证明旧发布；当前 0.1.10 fresh-install 结果见独立发布回执 |
| Docker 默认 Runtime source pin | npm `@glide-the/ink-claude-code-dream==0.1.10` | 解析 package-root `cli.js` 与相邻 manifest；精确版本/摘要不符时失败；本轮未构建镜像 |
...
版本结论：`2.1.241` 是 Dream 面向 Runtime 的 CLI/协议兼容标识，不表示官方全产品等价。当前实现依据是实际编译的原始 `src`、源码绑定的 headless/MCP 变换、manifest 和协议测试。当前同 SHA 发布与安装、本机启动采用及真实业务范围分别见执行回执；不同版本证据不能互相替代。完整目录与模块统一见[接入说明](../deploy/runtime-0.1.10-plugin-management-and-dream-adoption.md)。
...
当前源码与 Docker 精确依赖 `0.1.10`。实际服务采用哪个 Runtime，以该进程启动时的 resolver/manifest 回执为准，不能从磁盘文件或 PATH 变更推断。安装内容不可变，正常入口只选择已校验的 package-root `cli.js` 与匹配平台 payload；上一已验证安装可保留供明确回滚，不覆盖现用目录或 npm 版本。
...
provider-free 进程测试；真实业务列属于历史已资格化实现，不自动覆盖当前 0.1.10。
...
| `mcp.management.identity` | 正常生产图不使用 | 仅显式旧配置导入/独立回滚背景 | 不在 Resources 请求路径加载 | 无；Admin Registry134–147 是管理事实来源 | 正常页面 PID tracing 0；Dream service 无 production database import |
...
POST /api/claude-agent
  ...
  → SDK typed messages
  → Dream SSE EventBus
  → Admin DTO API → PostgreSQL Thread/Message/Run
  → Dream 本地 transcript 与共享文件系统
```
...
- Dream 使用标准 MCP SDK 的真实握手结果投影正交的连接与认证状态；Server 列表通过严格 Pydantic DTO 调用 Admin Registry134–147，由 Admin Zod DTO、Service 与 typed Drizzle Repository 读取，不等待远端 discovery，也不调用 Runtime `mcp list/get`。HTTP 类型本身不代表 OAuth，只有规范认证挑战和 metadata discovery 才进入授权；Admin 不可用时禁止回退 Dream PostgreSQL。
...
原始来源不是禁止输入，也不能隐藏成 MIT。构建收据必须声明 implementationRoot=src、implementationSource=repository 和 parallelImplementation=false；产物禁止 raw TypeScript、source map、Secret、Workspace 和用户数据。当前 `0.1.10` 发布状态见独立回执；任何新制品仍须验证其自身授权、资格和摘要。

### 10.2 npm 多平台布局

```text
@glide-the/ink-claude-code-dream@0.1.10
  ├─ cli.js：package-root selector
  └─ 根据 process.platform/process.arch 选择可选依赖

@glide-the/ink-claude-code-dream-darwin-arm64@0.1.10
@glide-the/ink-claude-code-dream-darwin-x64@0.1.10
@glide-the/ink-claude-code-dream-linux-arm64@0.1.10
@glide-the/ink-claude-code-dream-linux-x64@0.1.10
```

公开五包的精确 SHA-256、registry integrity、CI SHA 和安装结果统一记录在[发布与本机接入回执](../deploy/runtime-0.1.10-plugin-management-and-dream-adoption.md)，不在设计稿重复维护另一套摘要表。SBOM、source LICENSE、第三方 notices、manifest 和 checksum 必须描述实际制品；禁止 source map、源码树、用户数据和凭据进入发布包。
...
内存数据必须标注版本、制品摘要、宿主、场景、采样方法和范围；不同实现或初始化场景的样本不应变成当前产品容量承诺。此稿不保留废弃实现的对比数字，也不声称本轮测量了 Runtime `0.1.10` 的内存。真实 streaming、长 transcript、工具输出与外部 MCP 子进程须按目标业务单独评估。
...
- 当前 0.1.10 四个平台已通过 native-host qualification；未来功能版本须重新执行各宿主 sandbox/协议门，不能以格式检查或其他版本回执代替。
...
- [`docs/design/claude-mcp/mcp-apps-integration-strategy.md`](../design/claude-mcp/mcp-apps-integration-strategy.md)：MCP Apps technical preview 架构和 production-off 边界。
