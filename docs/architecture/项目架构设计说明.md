...
Registry130-132 进一步关闭 Dream launch Runtime provisioning 数据路径：Dream 请求边界把 current OAuth 注入 `AdminDreamLaunchRuntime`，以严格 current/replay Pydantic DTO 取得 Admin 派生的 current 或 Run-frozen binding/lock；Dream 只在共享 artifact store 校验 digest、manifest 与 Claude CLI。Admin 用 typed Drizzle Repository 执行 enabled Deck/Workspace/Voice 权限、Run/Preflight/binding 关系、materialization、current-mode Workspace installation 和 receipt。旧 `DreamRuntimeProvisioningService` 已删除；launch source、Preflight、Run、dispatch 和失败记录的其余生产 SQL 继续按后续阶段迁移。
