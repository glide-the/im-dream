...
| AutoDL SSH 发布 | [`autodl-ssh/deploy.sh`](autodl-ssh/deploy.sh)；[`autodl-ssh/runtime/start-ink-memory.sh`](autodl-ssh/runtime/start-ink-memory.sh) | SSH/rsync、直接本机 Python/Node 与 screen；验签并安装通用 Runtime 流程生成的 Linux x64 2.1.88 local-core 资格制品，制品内包含恢复后的 `vendor/seccomp` passthrough；生成 env 固定 `INK_AGENT_SANDBOX_ENABLED=false`，保留 Workspace Mode，但 approved Bash 以 Dream root 直接执行；Vite Preview 监听 `127.0.0.1:6006` 并代理 FastAPI `127.0.0.1:8765`，Admin upstream 为 `127.0.0.1:6008`；Dream workspace/Artifact/file/plugin/home 由幂等 initializer 固定到 AutoDL 数据盘，不使用 Docker/nginx |
