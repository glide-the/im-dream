...
| Remote SSH 发布 | [`remote-ssh/deploy.sh`](remote-ssh/deploy.sh) | 一键编排 SSH/rsync、主机 nginx、远端存储初始化、Docker Compose 构建启动与验证；默认写入生产 OAuth URL、Secure cookie 和前端域名 CORS；高级场景可用其 `setup-nginx` / `setup-storage` / `backup-data` / `sync-data` / `download-data` 子命令 |
