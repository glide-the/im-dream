...
Registry122-129 已关闭六个公开 Deck Plugin/Agent-type 数据路径：Dream 使用严格 Pydantic DTO 调用 Admin current/history/options/validate/save/clear 及 Runtime plan/prepare，Admin 用 typed Drizzle Repository 执行 owner、compatibility、CAS、历史、materialization 与 Workspace installation 持久化。Dream 保留公开页面、路由响应、Runtime 和共享文件系统，并用 Admin 返回的不含路径候选在本地校验 artifact、manifest 与 Claude CLI。公开 `agent-type` 不再打开 Dream 数据库；Dream launch 仍复用旧 provisioning 数据服务，继续按下一阶段迁移。
