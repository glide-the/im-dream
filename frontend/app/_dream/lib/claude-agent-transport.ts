 ...
 * [Pos]    transport adapter in frontend/app/_dream/lib
 ...
 * [Sync]   2026-09-06: retain the server-owned MCP App result projection in
 *                      AI SDK toolMetadata before the matching output settles.
 *
  ...
  switch (event.type) {
    ...
    case 'tool-output-available': {
      ...
      if (event.isError) {
        ...
        // AI SDK 6 retains tool metadata from the input part when the output
        // settles. Re-emit the already-normalized input immediately before
        // the output so the backend-authored identity survives the live path.
        if (event.mcpAppResult !== undefined) {
          chunks.push({
            type: 'tool-input-available',
            toolCallId: event.toolCallId,
            toolName: state.toolNames[event.toolCallId] ?? 'tool',
            input: state.toolInputs[event.toolCallId] ?? {},
            dynamic: true,
            toolMetadata: {
              mcpAppResult: event.mcpAppResult as JSONValue,
            },
          });
        }
        chunks.push({
