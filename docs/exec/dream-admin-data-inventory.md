<!-- [Sync] 2026-09-16: production pool/SQL closure complete in source; legacy database/schema/persistence remains test-only. -->
> 现行状态（2026-09-16）：下方 baseline 表保留迁移历史。Dream 生产 connection pool 和旧 SQL 模块已关闭；原 `backend/database.py`、`backend/persistence/`、`backend/schema/` 仅保存在 `backend/tests/legacy_*` 作为隔离 fixture。Story Workspace 最后一组生产 SQL 已由 Admin Registry185–191 的 DTO → Service → typed Drizzle Repository 替换。当前验收入口见 [生产数据库关闭阶段](../stage/stage_dream-production-database-closure.md) 与 [Story Workspace 消费阶段](../stage/stage_dream-story-workspace-artifact-consumer.md)。
...
| **backend/persistence/catalog.py**（历史路径，已退役或迁入测试隔离区） | production / 5 | LATERAL, pg_catalog.pg_am, pg_catalog.pg_attrdef, pg_catalog.pg_attribute, pg_catalog.pg_class, pg_catalog.pg_collation, pg_catalog.pg_constraint, pg_catalog.pg_index, pg_catalog.pg_namespace, pg_catalog.pg_proc, pg_catalog.pg_trigger, unnest | 详见 JSON 调用线索 | 需调用链核查 | 等待 Admin 规范；按原 aggregate 事务替换 |
| **backend/persistence/health.py**（历史路径，已退役或迁入测试隔离区） | production / 1 | 依赖/工厂入口 | 详见 JSON 调用线索 | 需调用链核查 | 等待 Admin 规范；按原 aggregate 事务替换 |
| **backend/persistence/postgres.py**（历史路径，已退役或迁入测试隔离区） | production / 0 | 依赖/工厂入口 | 详见 JSON 调用线索 | 需调用链核查 | 等待 Admin 规范；按原 aggregate 事务替换 |
| **backend/persistence/unit_of_work.py**（历史路径，已退役或迁入测试隔离区） | production / 1 | publishing | 详见 JSON 调用线索 | 需调用链核查 | 等待 Admin 规范；按原 aggregate 事务替换 |
...
| **backend/script/reconcile_story_artifact_index.py**（历史路径，已退役或迁入测试隔离区） | explicit import/maintenance script; verify callers before retirement / 0 | 依赖/工厂入口 | 详见 JSON 调用线索 | 需调用链核查 | 等待 Admin 规范；按原 aggregate 事务替换 |
| **backend/script/verify_gateway_e2e.py**（历史路径，已退役或迁入测试隔离区） | explicit import/maintenance script; verify callers before retirement / 8 | chat_message, chat_thread, gateway_requests, jsonb_array_elements, messages, platform_users, subscription_token_ledger_entries, subscription_usage_allowances, users | 详见 JSON 调用线索 | 549,592,727,783 | 等待 Admin 规范；按原 aggregate 事务替换 |
...
| **backend/services/claude_agent/remote_interaction_guard.py**（历史路径，已退役或迁入测试隔离区） | production / 2 | agent_sessions, workflow_runs | 详见 JSON 调用线索 | 需调用链核查 | 等待 Admin 规范；按原 aggregate 事务替换 |
| **backend/services/claude_agent/session_manager.py**（历史路径，已退役或迁入测试隔离区） | production / 17 | agent_sessions, deck_runtime_plugin_locks, runtime_load_receipt_entries, runtime_load_receipts, workflow_runs | L709 SessionManager._acquire_attempt: revision | 762,785,798,301,709,353,660 | 等待 Admin 规范；按原 aggregate 事务替换 |
...
| **backend/services/deck_plugin/installation_service.py**（历史路径，已退役或迁入测试隔离区） | production / 8 | columns, deck_plugin_installations, deck_plugin_releases, deck_runtime_plugin_locks | 详见 JSON 调用线索 | 需调用链核查 | 等待 Admin 规范；按原 aggregate 事务替换 |
...
| **backend/services/story_workspace/dream_workflow_lifecycle_service.py**（历史路径，已退役或迁入测试隔离区） | production / 0 | 依赖/工厂入口 | 详见 JSON 调用线索 | 需调用链核查 | 等待 Admin 规范；按原 aggregate 事务替换 |
...
| --- | --- | --- |
| **backend/database.py**（历史路径，已退役或迁入测试隔离区） | 144 | 5 |
...
| **backend/services/claude_agent/session_manager.py**（历史路径，已退役或迁入测试隔离区） | 18 | 0 |
| **backend/services/workflow/run_service.py**（历史路径，已退役或迁入测试隔离区） | 17 | 0 |
| **backend/services/story_workspace/artifact_story_index_repository.py**（历史路径，已退役或迁入测试隔离区） | 16 | 1 |
...
| **backend/services/runtime_plugin/reconcile_service.py**（历史路径，已退役或迁入测试隔离区） | 12 | 0 |
| `backend/services/deck/content_versioning.py`（历史基线，现已退役） | 11 | 1 |
| `backend/services/deck_plugin/release_service.py`（历史基线，现已退役） | 11 | 0 |
| **backend/services/deck_plugin/installation_service.py**（历史路径，已退役或迁入测试隔离区） | 7 | 0 |
| [backend/routers/claude_plugins.py](../../backend/routers/claude_plugins.py) | 6 | 0 |
| **backend/schema/capabilities.py**（历史路径，已退役或迁入测试隔离区） | 6 | 0 |
...
| **backend/services/claude_plugin/marketplace_service.py**（历史路径，已退役或迁入测试隔离区） | 4 | 0 |
...
| **backend/services/claude_agent/remote_interaction_guard.py**（历史路径，已退役或迁入测试隔离区） | 2 | 0 |
...
| **backend/services/story_workspace/artifact_story_index_reconcile.py**（历史路径，已退役或迁入测试隔离区） | 2 | 0 |
...
| **backend/persistence/postgres.py**（历史路径，已退役或迁入测试隔离区） | 0 | 0 |
