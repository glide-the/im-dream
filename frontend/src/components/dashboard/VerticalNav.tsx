...
export default function VerticalNav({ onToggleSidebar, onToggleFileSidebar, unreadCount = 0 }: VerticalNavProps) {
  return (
    <aside style={{ width: '4rem', flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '1.25rem 0.75rem', borderRight: '1px solid var(--color-border-paper)', background: 'var(--color-bg-app)' }}>
      <button type="button" onClick={onToggleSidebar} style={{ marginBottom: '1.75rem', display: 'grid', placeItems: 'center', width: '2.2rem', height: '2.2rem', border: 'none', borderRadius: '12px', background: 'var(--color-action-link)', color: '#fff', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'flex', flex: 1, flexDirection: 'column', alignItems: 'center', gap: '1.35rem', color: 'var(--color-text-muted)' }}>
        <button type="button" onClick={onToggleFileSidebar} title="Files" style={{ position: 'relative', border: 'none', background: 'transparent', color: 'var(--color-action-link)', cursor: 'pointer' }}>
          ...
          {unreadCount > 0 ? <span style={{ position: 'absolute', top: '-0.45rem', right: '-0.6rem', minWidth: '1rem', height: '1rem', padding: '0 0.2rem', borderRadius: '999px', background: 'var(--color-action-link)', color: '#fff', fontSize: '0.6rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{unreadCount}</span> : null}
        </button>
        ...
        <button type="button" style={{ border: 'none', background: 'transparent', color: 'inherit', cursor: 'pointer' }}><IconSettings style={{ width: '1.2rem', height: '1.2rem' }} /></button>
      </div>
