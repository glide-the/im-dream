...
// [Sync] 2026-09-14: preserve existing Apps request semantics after Browser session cutover.
...
test('Runtime receives resolved OAuth token and owned selectors without ambient credentials', async () => {
  ...
  const request = new Request('https://dream.example/api/mcp-apps/server-1', { method: 'POST', headers: {
    ...
    'x-ink-dream-credential': 'secret', 'x-user-id': '7', 'x-ink-mcp-apps-browser-session': 'opaque-session', 'x-ink-workspace-scope': 'owned-thread', 'mcp-session-id': 'upstream-session',
  ...
  for (const name of ['cookie', 'x-ink-csrf', 'x-ink-dream-credential', 'x-user-id']) assert.equal(result.headers.get(name), null);
