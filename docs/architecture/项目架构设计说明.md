...
<!-- [同步] 2026-08-28：Dream 源码/Docker 精确采用公开 Runtime 0.1.2；记录请求参数修复、same-SHA workflow、registry fresh install 和真实 Gateway/Admin 验收。 -->
...
>
> 发布状态：SDK `0.2.144` 已通过 Trusted Publisher 同时发布到 TestPyPI 与正式 PyPI；Dream 使用正式 PyPI 精确版本和 wheel/sdist SHA-256 锁。Runtime `main@c3e4d4e2f74960c75b42b1cd48adedf90345a10b` 的五个 `0.1.2` npm 包均已公开并通过 provider-free registry fresh install；最小五包权限 npm token 与 GitHub `npm` Environment Secret 按用户要求保留。
>
...
| clean-room Runtime package | `@glide-the/ink-claude-code-dream@0.1.2` | npm 公开 selector；对 Dream 输出 `2.1.241 (Claude Code)` 兼容标识；manifest 配对 SDK `0.2.144` |
| Runtime 仓库 | release `main@c3e4d4e2f74960c75b42b1cd48adedf90345a10b` | qualification run `33149053281` 与 publish run `33151128000` 均成功；发布记录随后由 PR #14 合入 main |
| Runtime 实现输入 | `src/cleanroom/` 48 个文件、290,209 字节；tree SHA-256 `6e36ce5c67f05d7e752a117bfb3ed097dc9611c4a0aa0b3c5f675f9ea984a090` | 仓库自有 MIT 代码；禁止输入扫描不含 restored/vendor 代码 |
...
| registry 验收 Runtime | selector integrity `sha512-JZnQ8u…cqZVg==`；darwin-arm64 integrity `sha512-K1Hkz…Z9zZw==` | fresh install 只选择当前平台包；两个 CLI alias、manifest 配对与零 `.map` 通过 |
| Runtime executable | darwin-arm64 SHA `44eb30d4…d3fef54` | `0.1.2` 公开平台包执行对象 |
| Docker 默认 Runtime | npm `@glide-the/ink-claude-code-dream==0.1.2` | 先装 selector/匹配 Linux 平台包；构建时执行版本、manifest 与 Dream resolver 门 |
| 本机现有服务 Runtime | PATH 软链接仍为 `0.1.1` | 为避免重启或热切换用户服务，本轮不改该软链接；源码合并和下一次正常构建/启动采用 `0.1.2` |
...
```text
@glide-the/ink-claude-code-dream@0.1.2
  └─ selector：根据 process.platform/process.arch 选择可选依赖

@glide-the/ink-claude-code-dream-darwin-arm64@0.1.2
@glide-the/ink-claude-code-dream-darwin-x64@0.1.2
@glide-the/ink-claude-code-dream-linux-arm64@0.1.2
@glide-the/ink-claude-code-dream-linux-x64@0.1.2
```

| 包 | npm registry tgz SHA-256 | registry 大小 |
| --- | --- | ---: |
| selector | `0d6ed5371614b478b57fe192ff5555537279d790ae552818cd4b0b307e24ddc3` | 18,405 B |
| darwin-arm64 | `2c39bf8146ebcc65f3f1ecb55d969b722ad9779473398d41c784cb88caa8eeb2` | 27,006,615 B |
| darwin-x64 | `7050576a5b809e6662bc791138efb28a04fa56d053d220cfb892034b4b36f8e9` | 29,157,799 B |
| linux-arm64 | `abc209eccd9ace4ad20050db9f04d587959560e48fc3373f5b478288c5f80da2` | 37,968,588 B |
| linux-x64 | `0af763146bf0f5656baa552445e8953de18cec7650938a5060d1d6cd62bdd53b` | 37,472,361 B |
...
五个包已按四个平台包优先、selector 最后的顺序发布到 npm。公共 packument 均返回 `0.1.2`、MIT 和对应 `os/cpu`；全新目录安装只选择 Darwin ARM64 平台包，`ink-claude-code-dream --version`、`claude --version`、manifest 配对和零 `.map` 检查全部退出 0。表中 SHA-256 由公共 registry tarball 重新下载计算；publish job 只下载、复验并发布同 SHA qualification 制品，没有重新构建。
...
| Dream 相关后端回归 | 既有完整回归 `592 passed, 1 skipped, 181 subtests passed`；本次 SDK/Runtime 版本接入聚焦 `96 passed, 6 subtests passed` |
...
| Runtime 测试 | release CI `117 total, 113 passed, 4 conditional external-fixture skips, 0 fail`；lint exit 0 |
...
| npm registry fresh install | `0.1.2` selector + darwin-arm64；两个 CLI alias `--version` 为 `2.1.241 (Claude Code)`；manifest 配对 SDK `0.2.144`；无 `.map`；provider-free wheel/sdist 双路径均通过 |
| Runtime 请求字段真实验收 | 同一 Thread 首轮/resume 的 Gateway 请求均为 `max_tokens=32000`、`output_config.effort=low`、`stream=true`、Authorization `[REDACTED]`；Playwright `1 passed (15.4s)`；Admin 只读验收 `1 passed (4.4s)` |
