...
- Branch：`codex/mcp-apps-phase0-3-sol`。
...
| Official compatibility + lifecycle | `pnpm exec playwright test e2e/mcp-apps/phase-1/official-phase1-3.spec.ts --reporter=line --workers=1`（从 `frontend/`，显式 current/previous artifact roots） | `0`；`3 passed (9.9s)`；官方 `1.7.4/1.7.5` protocol smoke 与 `1.7.5` production-module Host 生命周期通过。 |
...
- Phase 1 的页面 tool call 保持拒绝；AppBridge 不持有可自动转发的 SDK Client。Phase 2 只手动开放服务端正向列表中的低风险工具；高风险、需确认和未分类工具在解密/上游调用前拒绝。
