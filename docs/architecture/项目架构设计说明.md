...
> 代码证据：`54f3bbe539b086035bce5900618bf3d7ae5b9372`（2026-09-06）。SDK/Runtime registry 证据日期：2026-08-30。
>
...
旧 `0.1.0`—`0.1.3` qualification/registry 安装树只保留历史回滚证据。当前源码与 Docker 精确依赖
公开 `0.1.4`；实际服务是否已继承新 PATH 仍以该受控进程启动时的 resolver/manifest 回执为准，不能从磁盘安装树推断：
