...
是。当前仓库已经存在三类发布路径，但文档入口没有清晰分层：

| 发布方式 | 当前承载位置 | 判断 |
|----------|--------------|------|
| Google Cloud 发布 | [`../../deploy/google-cloud/deploy.sh`](../../deploy/google-cloud/deploy.sh)、旧 `deploy/setup-storage.sh`、`deploy/setup-env.sh`、`deploy/deploy.sh`、`deploy/sync-data.sh`、`docs/deploy/overview.md`、`docs/deploy/data-sync.md` | 已形成目录化入口，旧根脚本保留兼容 |
...
结论：`deploy/` 脚本层应从 Google Cloud Run 专属目录升级为平台化发布入口；旧 Cloud Run 根脚本继续保留，新的 `deploy/google-cloud/` 只做目录化包装，避免破坏既有调用路径。

### 主要问题

1. **文档职责混杂**：`docs/deploy/overview.md` 标题和主体是 Google Cloud Run，但结尾包含“本地 Docker Compose 运行”；本地直跑流程仅在 README，发布文档入口没有分流。
2. **环境变量说明与脚本实现不一致**：`overview.md` 描述的 Secret Manager 变量是 `TEXT_API_KEY`、`INK_IMAGE_API_KEY`、`JWT_SECRET_KEY`，而当前 [`../../deploy/setup-env.sh`](../../deploy/setup-env.sh) 实际确认的是 `ANTHROPIC_*`、`AGENT_CWD`、`FILE_STORAGE_LOCAL_DIR` 等 8 个 key，并将其他 `backend/.env` key 透传为 Cloud Run 普通环境变量。
...
4. **数据同步文档与脚本行为不一致**：`data-sync.md` 写了数据库、`file-storage/`、`agent-workspace/` 的手动同步；[`../../deploy/sync-data.sh`](../../deploy/sync-data.sh) 只上传 SQLite 主文件和 WAL/SHM 文件并重启后端，未同步两个目录。
5. **脚本文案存在轻微误导**：`setup-storage.sh` 结束提示 `run ./deploy.sh`，实际路径应为 `./deploy/deploy.sh`。
6. **运行时说明滞后**：`overview.md` 描述后端镜像为 `python:3.9-slim`，实际 [`../../backend/Dockerfile`](../../backend/Dockerfile) 是 `python:3.10-slim-bookworm`；前端构建文档写 `npm ci`，实际 [`../../frontend/Dockerfile`](../../frontend/Dockerfile) 使用 `npm install`。
...
| 层级 | 处理方式 | 说明 |
|------|----------|------|
| 文档层 | 拆分入口与三类发布说明 | `docs/deploy/README.md` 做分流；后续将 `overview.md` 拆为 `google-cloud.md`、`docker.md`、`local.md` |
| 脚本层 | 新增平台子目录，保留旧云脚本兼容 | `deploy/local/`、`deploy/docker/`、`deploy/google-cloud/` 提供统一入口；旧根脚本继续可用 |
...
| 对象 | 建议 | 理由 |
|------|------|------|
| `deploy/` | 升级为平台化发布脚本目录 | 顶层做入口说明，子目录承载平台脚本，旧 Cloud Run 根脚本兼容保留 |
...
| `deploy/google-cloud/` | 新增并保留 | Cloud Run 目录化入口，包装旧 `deploy/*.sh` |
...
| `docs/deploy/README.md` | 新增 | 作为三类发布路径分流入口 |
...
flowchart TD
  ...
  D --> D3["nginx BACKEND_URL 指向 Compose 服务名"]

  G --> G1["deploy/google-cloud/deploy.sh + legacy deploy/*.sh"]
...
- 需要复现 Cloud Run 的“前端 nginx 代理后端 API”形态，但不创建云资源。
- 适合本地验收、自托管试运行、运维演练。
...
| 前端代理目标 | `docker-compose.yml` 的 `BACKEND_URL=http://ink-backend:8765` | 指向 Compose 内部服务名 |
...
- 前端发起 `/ink-and-memory/api/` 请求时，由 nginx 代理到后端容器。
...
| 前端能打开但 API 失败 | 检查 `BACKEND_URL` 是否为 Compose 服务名，不应填 localhost |
| 后端缺少模型或密钥 | 检查 `backend/.env` 和 `backend/models.json` 是否挂载/传入 |
| 数据丢失 | 确认 `backend/data/` 被持久化挂载，不要删除本地目录 |

### 与其他发布方式的边界

- 不执行 `deploy/setup-storage.sh`、`deploy/setup-env.sh`、`deploy/deploy.sh`。
...
| 前端 API 跨域或路径异常 | 检查前端 API 基础路径和后端端口，避免混用 Cloud Run URL |
...
- 需要通过 Cloud Run URL 访问 `/ink-and-memory/`。
...
| 存储资源 | `.storage-env` | 由 `setup-storage.sh` 生成，包含 `GCS_BUCKET`、`SA_EMAIL` |
...
| 前端代理 | Cloud Run 后端 URL | `deploy.sh` 部署后端后注入前端 `BACKEND_URL` |

### 构建流程

1. `deploy/deploy.sh` 设置 GCP 项目并启用 Cloud Run、Artifact Registry、Cloud Build API。
...
- `/ink-and-memory/api/` 请求能通过前端 nginx 代理到后端。
...
| 问题 | 处理 |
|------|------|
| `.storage-env` 缺失 | 先运行 `./deploy/setup-storage.sh` |
| `.cloud-env` 缺失 | 先运行 `./deploy/setup-env.sh` |
| 前端 API 代理失败 | 检查后端 Cloud Run URL 是否注入 `BACKEND_URL`，以及 nginx 模板是否只替换 `${BACKEND_URL}` |
...
当前仓库已经具备 Cloud Run 发布脚本、Docker Compose 容器启动、本地直跑说明三种能力，但文档结构把它们放在不同位置且边界不一致。团队成员在选择发布方式时容易出现三类误用：
...
- 建立本地、Docker、Google Cloud 三类发布路径的清晰边界。
...
├── google-cloud/
│   ├── .folder.md
│   └── deploy.sh             # Cloud Run 目录化入口，包装旧脚本
├── setup-storage.sh          # 兼容：Google Cloud 存储与服务账号初始化
├── setup-env.sh              # 兼容：Google Cloud env/secret 初始化
├── deploy.sh                 # 兼容：Google Cloud 镜像构建、推送、Cloud Run 部署
├── sync-data.sh              # 兼容：Google Cloud 数据上传与后端重启
└── .folder.md                # 脚本目录说明
```
...
| `deploy/google-cloud/deploy.sh` | 已新增：包装旧 Cloud Run 脚本，提供 plan/check/setup/deploy/sync/verify/rollback/clean | 已完成 |
...
| `sync-data.sh` | 明确只同步 SQLite，或增加 `file-storage/`、`agent-workspace/` 同步选项 | 高 |
| `setup-storage.sh` | 修正结束提示路径为 `./deploy/deploy.sh` | 低 |
| `deploy.sh` | 镜像 tag 从固定 `latest` 扩展为可配置 `IMAGE_TAG`，默认仍可用 `latest` | 中 |
| `docker-compose.yml` | 端口、`BACKEND_URL` 保持默认，但文档说明如何覆盖 | 中 |
...
3. 然后执行对应验证：本地端口、容器健康检查、Cloud Run URL 与 GCS。
...
| Docker 与 Cloud 共享 nginx 模板 | 本地改动影响云端代理 | 修改模板必须同时验证 Compose 和 Cloud Run 代理 |
...
- `bash -n deploy/*.sh` 做 shell 语法检查。
- Cloud dry-run 或 staging 项目试跑 `setup-env.sh`、`deploy.sh`。
...
| 阶段 | 内容 | 验收 |
|------|------|------|
| Phase 1 | 新增发布体系设计稿和 README，更新 `.folder.md` | 文档路径有效，团队可以按三类路径分流 |
| Phase 2 | 拆出 `local.md`、`docker.md`、`google-cloud.md` | README、overview、三类文档不重复描述同一流程 |
