...
<!-- [同步] 2026-09-12：当前源码合同升级为 SDK 0.2.145 × 未发布 Runtime 0.1.6 package-root cli.js；0.1.4 证据降为历史。 -->

# 项目架构设计说明

> 前端代码证据：`54f3bbe539b086035bce5900618bf3d7ae5b9372`（2026-09-06）。Runtime registry 查询日期：2026-09-12。
>
> 当前状态：Dream 源码使用自有 Python SDK `0.2.145`，默认 Runtime 合同为未发布的 clean-room `0.1.6` package-root selector；官方 Claude CLI 只作为显式绝对路径回滚。本文不据源码推断正在运行的服务版本。
>
> 发布状态：Dream 使用正式 PyPI 精确版本和 wheel/sdist SHA-256 锁固定 SDK `0.2.145`。npm registry 在 2026-09-12 的 `latest` 仍为 `0.1.4`；`0.1.6` 的 production、redistribution、publication 与四 target gate 全部关闭，旧 `0.1.5` acceptance 不复用。
>
...
| clean-room Runtime package | source contract `@glide-the/ink-claude-code-dream@0.1.6` | `package/cli.js` selector；对 Dream 输出 `2.1.241 (Claude Code)`；manifest 配对 SDK `0.2.145`；尚未发布 |
| Runtime 历史发布 | `0.1.4` / `main@0ebafe95db22101cf77db2c27e73b561d3af37a6` | qualification `33306855166` 与 publish `33306940462` 仅为旧制品证据 |
| Runtime 实现输入 | `package/` selector 源 + `src/cleanroom/` MIT Runtime + 兼容许可证依赖 | 禁止输入 restored/vendor source、source map 与用户数据；实际新 tree/digest 由 0.1.6 qualification 生成 |
...
| registry 验收 Runtime（历史） | 0.1.4 source tree `266362ac…95de81`；darwin-arm64 executable `969f9193…980dd` | 只证明旧发布；0.1.6 必须产生新 fresh-install 回执 |
| Docker 默认 Runtime source pin | npm `@glide-the/ink-claude-code-dream==0.1.6` | 解析 package-root `cli.js` 与相邻 manifest；发布前精确安装 fail closed |
...
未发布 `0.1.6`；实际服务是否已继承某个 PATH 仍以该受控进程启动时的 resolver/manifest 回执为准，不能从磁盘安装树推断：
...
```text
@glide-the/ink-claude-code-dream@0.1.6
  ├─ cli.js：package-root selector
  └─ 根据 process.platform/process.arch 选择可选依赖

@glide-the/ink-claude-code-dream-darwin-arm64@0.1.6
@glide-the/ink-claude-code-dream-darwin-x64@0.1.6
@glide-the/ink-claude-code-dream-linux-arm64@0.1.6
@glide-the/ink-claude-code-dream-linux-x64@0.1.6
```
