...
| 隐藏launch来源与dispatch | 原[dream_launch_application_service](../../backend/services/story_workspace/dream_launch_application_service.py)/[dream_launch_infrastructure](../../backend/services/story_workspace/dream_launch_infrastructure.py) | 候选dream-launch-source.ensure/dream-launch-dispatch.claim/finish尚未在72注册，不接生产消费者；原source→PF/Run→claim COMMIT→Voice/Runtime→独立finish语义按源码核对 | 候选source23+ingress5/dispatch32仅Admin报告技术gate；prepare/Agent-model-binding/failure recorder/publicPG/full normal launch与当前SQL仍pending |
