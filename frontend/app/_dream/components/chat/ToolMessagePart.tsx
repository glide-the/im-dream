...
// [Pos] tool-message-part component node in frontend/app/_dream/components/chat
...
import type { SendMcpAppUserMessage } from './mcp-apps/ImMcpAppHostAdapter';
...
export function ToolMessagePart({ part, threadId, isLast, isLoading, addToolResult, sendUserMessage, onEditorWriteConfirmed, settledToolCallIds, onConfirmationSettled }: ToolMessagePartProps) {
  ...
  const partWithMcpMetadata = part as unknown as {
    mcpAppResult?: unknown;
    toolMetadata?: { mcpAppResult?: unknown };
  };
  const mcpAppResult = partWithMcpMetadata.mcpAppResult
    ?? partWithMcpMetadata.toolMetadata?.mcpAppResult;
  const savedMcpAppCall = useMemo(() => parseSavedMcpAppToolCall({
    mcpAppResult,
    output,
    toolName,
    toolCallId,
    input,
  }), [input, mcpAppResult, output, toolCallId, toolName]);
  const sendMcpAppMessage = useCallback<SendMcpAppUserMessage>(async (content) => {
    if (!sendUserMessage || content.length === 0) {
      throw new Error('MCP App message content is unavailable.');
    }
