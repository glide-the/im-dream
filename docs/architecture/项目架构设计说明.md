...
Registry121 关闭 confirmation dispatcher 的 Runtime 数据回退：Dream 用严格 Pydantic `claim-turn` 消费 Admin 在同一事务生成的 claim-bound `server-persistence` grant，并复用既有 Workflow、Deck 与 `AdminTurnPersistence` DTO 客户端。Dream 仍执行同 Thread Runtime、EventBus、SSE 和共享文件操作；Admin 执行消息 claim、主体/实体权限、事务和全部持久化。其他生产入口继续按迁移清单逐项关闭。
...
实施状态以[执行计划](../exec/dream-admin-auth-data-plan.md)为准，设计契约和局部client源码不等于已发布能力或真实业务验收。下文仍记录baseline实际实现/兼容面，Python认证与DB直连说明和最新目标冲突的部分必须随各领域迁移更新；未迁完不能声称完成。
