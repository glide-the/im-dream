...
<!-- [Sync] 2026-08-30: replace the old unavailable CLI placeholder with installation/connection-aware availability. -->
<!-- [Sync] 2026-08-30: remove the hero subtitle and make Skill rows/files catalog-driven rather than skeleton constants. -->
...
- 事实：当前安装 `notion-session` 与 `notion-cli`；Read hook 可用，CLI 通过 actor/thread-bound Runtime 环境执行，Hosted Notion MCP 未接入
...
│                   │ effective / 下次计划                                │
│──────────────────────────────────────────────────────────────────────────│
│ Skills            │ [Skill] {catalog.skills[0].title} 内置 · {状态}   ›  │
│                   │ [Skill] {catalog.skills[n].title} 内置 · {状态}   ›  │
│──────────────────────────────────────────────────────────────────────────│
...
- 页首标题固定为“Notion CLI”，标题下不显示用途小字；Notion 仍是 provider/连接动作名称。
...
┌────────────────────────── Skill 子页 ──────────────────────────┐
│ ← Notion                                                         │
│ {当前 Skill 的 catalog title}             [内置] [{真实可用性}] │
│ {对应 SKILL.md frontmatter description}                          │
│ （无启用 / 停用开关）                                             │
│──────────────────────────────────────────────────────────────────│
│ Skill 说明                                                        │
...
│──────────────────────────────────────────────────────────────────│
│ 包含的文件                                                        │
│ {当前 Skill 动态 reference 文件}  text/markdown · {大小}      ›  │
│ …                                                               │
└──────────────────────────────────────────────────────────────────┘
```
...
```text
← 包含的文件
{当前 Skill title} / {动态 relative path}
...
- 只显示相对路径、MIME 和大小，不在该子页展示 Runtime 配置。
...
- 保存/同步只刷新受影响的服务器状态；Skill 局部失败不触发整个 connector 重载。
...
错误统一回答：发生了什么、系统是否自动恢复、用户下一步。

## 9. 结构验收

1. 页首后严格为“权限 → Skills → 读取操作 → 写入操作 → 资源范围 → 已挂载来源 → 信息”，信息在自然滚动末端。
2. 主详情和四个子页只有 Settings 内容区一条纵向滚动。
3. Skills 区逐行渲染 capability catalog，不在页面或 workspace renderer 维护静态 Skill 清单；当前制品返回 `notion-session` 与 `notion-cli`。各 Skill 上方显示自己的正文、下方显示自己的动态 reference 文件，无开关。
4. 读取和写入分别对应真实 Read Hook 与 workspace materializer；后者不写回 Notion。
5. 资源与来源长列表、统计和状态只在子页，主详情分别只有一个管理入口。
