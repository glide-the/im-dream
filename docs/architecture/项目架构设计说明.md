<!-- [输入] Dream 当前代码、Next.js/pnpm workspace、Admin/Gateway/PostgreSQL 合同、自有 Claude Agent SDK main 与 clean-room Runtime main/npm registry 证据。 -->
...
<!-- [同步] 2026-09-13：当前源码合同升级为 SDK 0.2.145 × 未发布 Runtime 0.1.8 package-root cli.js；0.1.4 证据降为历史。 -->

# 项目架构设计说明

> 前端代码证据：`54f3bbe539b086035bce5900618bf3d7ae5b9372`（2026-09-06）。Runtime registry 查询日期：2026-09-13。
>
> 当前状态：Dream 源码使用自有 Python SDK `0.2.145`，默认 Runtime 合同为未发布的原始模块统一实现 `0.1.8` package-root selector；官方 Claude CLI 只作为显式绝对路径回滚。本文不据源码推断正在运行的服务版本。
>
> 发布状态：Dream 使用正式 PyPI 精确版本和 wheel/sdist SHA-256 锁固定 SDK `0.2.145`。npm registry 在 2026-09-13 的 `latest` 仍为 `0.1.4`；`0.1.8` 的 production、redistribution、publication 与四 target gate 全部关闭，旧 `0.1.5` acceptance 不复用。
>
...
- Runtime 位于独立仓库 `ink-claude-code-dream`，实际实现为原始 `src`，与 `restored-src/src` 同目录、同模块、同初始字节/权限。
- 默认构建读取 `src/entrypoints/cli.tsx`，不读另一套外部 src，也不编译已删除的 `src/cleanroom`。原始源码版权和制品来源如实保留，公共发布门未开放。
...
| 原始模块 Runtime | JSONL、Provider/tool loop、resume、MCP、Workspace、sandbox | 不包含 IM 业务代码或用户可变数据；source-derived 发布资格尚未完成 |
...
| 对象 | 当前版本或身份 | 当前适用性 |
| --- | --- | --- |
| Dream 项目元数据 | backend `0.1.2`、frontend `0.0.2`；API schema `2.0.0` 不变 | 现有项目版本序列的 patch 更新，不代表部署或 API 变更 |
...
| 原始模块 Runtime package | source contract `@glide-the/ink-claude-code-dream@0.1.8` | `package/cli.js` selector；对 Dream 输出 `2.1.241 (Claude Code)`；manifest 配对 SDK `0.2.145`；尚未发布 |
| Runtime 历史发布 | `0.1.4` / `main@0ebafe95db22101cf77db2c27e73b561d3af37a6` | qualification `33306855166` 与 publish `33306940462` 仅为旧制品证据 |
| Runtime 实现输入 | 原始 `src`，默认入口 `src/entrypoints/cli.tsx` + recovered dependencies/vendor assets | 实际编译原始模块，headless/MCP 变换在构建层；旧 `src/cleanroom` 已退出；source map 和用户数据不进入制品 |
| Runtime 参考源码 | 原始 `restored-src/src`：1,902 文件、35 模块目录；subtree `7640f58e…ebe30` | 与 src 同目录、同模块、同初始字节/权限；Anthropic 版权保留；同一 Runtime，不是另设计一套实现 |
...
| registry 验收 Runtime（历史） | 0.1.4 source tree `266362ac…95de81`；darwin-arm64 executable `969f9193…980dd` | 只证明旧发布；0.1.8 必须产生新 fresh-install 回执 |
| Docker 默认 Runtime source pin | npm `@glide-the/ink-claude-code-dream==0.1.8` | 解析 package-root `cli.js` 与相邻 manifest；发布前精确安装 fail closed |
...
版本结论：`2.1.241` 是 Dream 面向 Runtime 的 CLI/协议兼容标识，不表示官方全产品等价。当前候选实现依据是实际编译的原始 src 模块、源码绑定的 headless/MCP 变换、manifest 和协议测试；新同 SHA 真实业务/发布回执仍缺失，不能复用旧 MIT 实现的证据。完整目录与模块统一见[接入说明](../deploy/runtime-0.1.8-source-structure-integration.md)。
...
旧 `0.1.0`—`0.1.5` qualification/registry/acceptance 只保留历史证据。当前源码与 Docker 精确依赖
未发布 `0.1.8`；实际服务是否已继承某个 PATH 仍以该受控进程启动时的 resolver/manifest 回执为准，不能从磁盘安装树推断：
...
~/.local/share/ink-claude-code-dream/releases/
  ...
  npm-registry-0.1.4-<content-digest>/
    node_modules/@glide-the/ink-claude-code-dream/
    node_modules/@glide-the/ink-claude-code-dream-darwin-arm64/

~/.local/bin/ink-claude-code-dream
  -> releases/npm-registry-0.1.4-<content-digest>/.../bin/ink-claude-code-dream
```
...
provider-free 进程测试；真实业务列属于历史已资格化实现，不自动覆盖当前 0.1.8。
...
原始来源不是禁止输入，也不能隐藏成 MIT。构建收据必须声明 implementationRoot=src、implementationSource=repository 和 parallelImplementation=false；产物禁止 raw TypeScript、source map、Secret、Workspace 和用户数据。实际 source-derived 多平台发布仍缺授权/资格。

### 10.2 npm 多平台布局

```text
@glide-the/ink-claude-code-dream@0.1.8
  ├─ cli.js：package-root selector
  └─ 根据 process.platform/process.arch 选择可选依赖

@glide-the/ink-claude-code-dream-darwin-arm64@0.1.8
@glide-the/ink-claude-code-dream-darwin-x64@0.1.8
@glide-the/ink-claude-code-dream-linux-arm64@0.1.8
@glide-the/ink-claude-code-dream-linux-x64@0.1.8
```
...
历史 `0.1.4` 五个包曾按四个平台包优先、selector 最后的顺序发布到 npm。公共 packument 均返回 `0.1.4`、MIT 和对应 `os/cpu`；全新目录安装只选择 Darwin ARM64 平台包，`ink-claude-code-dream --version`、`claude --version`、manifest 配对、`sandbox.notion-cli` 和零 `.map` 检查全部退出 0。表中 SHA-256 来自 registry 匿名 `npm pack`；该次 registry 字节与 qualification 生成的五包 SHA-256 一致，publish job 没有重新构建。此证据不覆盖当前未发布 `0.1.8`。

### 10.3 发布判断

历史已公开的 Runtime `0.1.4` 是仓库自有 clean-room MIT 实现，不是通过删除一个许可证文件把恢复源码改名发布。恢复源码和旧派生 bundle 不在该次发布输入中，因此其再分发限制不适用于那五个 clean-room 包；当前 `0.1.8` 仍必须以自身字节重新通过仓库 license/SBOM/prepack/真实业务/registry 回下载门。

SDK wheel/sdist 与 Runtime npm 是两条独立发布链。Bun 只构建 JavaScript/TypeScript Runtime，不替代 Python `pyproject.toml`、wheel 或 sdist。

历史正式 PyPI `0.2.144` 制品为：
...
下表记录的是 Python Claude Agent、managed MCP、clean-room Runtime、Gateway、Workspace 与 Dream Workflow 的既有发布/真实业务证据。它们没有经过 Browser Host → Next Node Runtime → 外部 App Server 这条 MCP Apps 链，因此不能补足 13.1 的真实外部 Server/OAuth、公开应用或 production gate。
