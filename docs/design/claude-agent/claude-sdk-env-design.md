...
<!-- [同步] 2026-09-13：源码合同升级到 SDK 0.2.145 × Runtime 0.1.7；npm 接受 package-root cli.js，另行保留严格资格化的 AutoDL local-core 布局。 -->
...
> 门禁的 `ink-claude-code-dream==0.1.7`；npm 默认布局为 package-root `cli.js`，AutoDL local-core 保留独立受检布局。`CLAUDE_CODE_CLI_PATH` 是唯一显式
...
> CLI 回滚物。Runtime `0.1.7` 是未发布源码候选；2026-09-13 registry `latest` 仍为已完成五包验收的 `0.1.4`。
> 当前源码不能与 `0.1.4` 混用，Docker 在 `0.1.7` 正式发布前会在精确 npm 安装处 fail closed。AutoDL local-core 是独立、不可公开分发的制品，不冒充 clean-room 五包。
...
当前 Dream 源码固定的是 clean-room `@glide-the/ink-claude-code-dream@0.1.7` 候选：
可发布制品的源码只来自 Runtime 仓库自有 MIT `src/cleanroom/` 和兼容许可证依赖，
