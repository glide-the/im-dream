...
> **父 Issue**: SUO-198  
> **设计增量来源**: SUO-214（DEC-009～DEC-011）、SUO-215（Deck/Desk/Claude Agent 集成 Delta）、**SUO-230（DEC-017～DEC-018：Dream 导航与审阅 Gate）**  
> **生成 Agent**: IssueDispatcher  
> **最后更新**: 2026-08-01（SUO-230 增量传播完成）  
> **更新类型**: 增量差异，不重写稳定基线
...
- 来源设计稿：
  ...
  - 增量 Delta 设计稿：`docs/design/story-workspace/story-workspace-deck-desk-integration-delta.md`
...
- 本清单覆盖范围：
  ...
  - Desk 配置预检：解析 Agent 提示词/插件配置就绪状态，执行前强制校验
...
- 关键约束：
  ...
  - Desk、Deck Editor、Deck Plugin 与 story-workspace 保持独立职责，不合并术语或数据所有权（DEC-009）
  - 每次创作锁定 Deck 插件版本与 Desk 配置快照引用；切换只影响新执行（DEC-010）
  - 未选择/不可用 Deck 插件或 Desk 配置不完整时禁止启动 Claude Agent（DEC-011）
  - Ink-Dream 不存储 Desk 密钥或完整提示词正文，只保存版本化引用和脱敏摘要（DEC-012）
...
- 补充说明：
  ...
  - SUO-214 / SUO-215 设计增量引入 Deck/Desk/Agent 端到端集成，需新增对应 Issue
...
|---|---|---|---|---|---|---|---|
| `SUO-201-BE-001` | 数据库 Schema 与数据表初始化 | backend | P0 | `database`,`schema`,`migration` | 无 | `@BackendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-BE-002` | Story Workspace REST API 实现 | backend | P0 | `api`,`rest`,`crud` | `SUO-201-BE-001` | `@BackendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-BE-003` | 审阅状态流转与批量操作 API | backend | P0 | `api`,`review`,`workflow` | `SUO-201-BE-002` | `@BackendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-BE-004` | Agent 产出数据接收与存储集成 | backend | P0 | `agent`,`integration`,`sse` | `SUO-201-BE-001` | `@BackendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-FE-001` | 三栏布局骨架与全局样式 | frontend | P0 | `layout`,`ui`,`desktop-only` | 无 | `@FrontendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-FE-002` | Sidebar 导航与路由配置 | frontend | P0 | `navigation`,`router`,`sidebar` | `SUO-201-FE-001` | `@FrontendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-FE-003` | 数据表格组件（故事/角色/场景） | frontend | P0 | `table`,`component`,`data-display` | `SUO-201-FE-002` | `@FrontendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-FE-004` | 审阅面板（Review Panel）与审阅操作 | frontend | P0 | `review`,`panel`,`workflow` | `SUO-201-FE-003` | `@FrontendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-FE-005` | 工作台首页 Dashboard | frontend | P1 | `dashboard`,`overview` | `SUO-201-FE-003` | `@FrontendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-FE-006` | 空态/加载/错误/选中态组件 | frontend | P1 | `state`,`ui`,`ux` | `SUO-201-FE-003` | `@FrontendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-SH-001` | 前端-后端联调：审阅工作流 E2E | shared | P0 | `e2e`,`integration`,`review-workflow` | `SUO-201-BE-003`, `SUO-201-FE-004` | `@FrontendTaskAgent` + `@BackendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-SH-002` | 命名规范与类型定义共享包 | shared | P0 | `naming`,`types`,`shared` | 无 | `@FrontendTaskAgent` + `@BackendTaskAgent` | ✅ 基线稳定 |
| `SUO-201-DO-001` | Story Workspace 使用文档 | docs | P2 | `documentation`,`user-guide` | `SUO-201-SH-001` | `@FrontendTaskAgent` | ✅ 基线稳定 |

### 2.2 增量新增/变更 Issue（由 SUO-214/SUO-215 设计增量引入）

| Issue ID | 标题 | 类型 | 优先级 | 标签 | 前置依赖 | 分发去向 |
|---|---|---|---|---|---|---|
| `SUO-226-BE-001` | Workflow Binding 与 Run 数据模型 | backend | P0 | `database`,`schema`,`workflow-run`,`delta` | `SUO-201-BE-001` | `@BackendTaskAgent` |
| `SUO-226-BE-002` | Deck 插件目录查询与选择 API | backend | P0 | `api`,`deck-plugin`,`directory` | `SUO-201-BE-002` | `@BackendTaskAgent` |
| `SUO-226-BE-003` | Desk 配置预检与就绪状态 API | backend | P0 | `api`,`desk-config`,`preflight` | `SUO-226-BE-002` | `@BackendTaskAgent` |
| `SUO-226-BE-004` | Workflow Run 创建与执行上下文 API | backend | P0 | `api`,`workflow-run`,`agent-context` | `SUO-226-BE-001`, `SUO-226-BE-003` | `@BackendTaskAgent` |
| `SUO-226-BE-005` | Agent 集成适配：带锁定上下文的执行触发 | backend | P0 | `agent`,`integration`,`deck-desk` | `SUO-201-BE-004`, `SUO-226-BE-004` | `@BackendTaskAgent` |
| `SUO-226-FE-001` | Dashboard Deck 工作流上下文条 | frontend | P0 | `dashboard`,`deck-selector`,`workflow-context` | `SUO-201-FE-005` | `@FrontendTaskAgent` |
| `SUO-226-FE-002` | 审阅面板来源溯源与版本信息展示 | frontend | P1 | `review`,`provenance`,`version` | `SUO-201-FE-004` | `@FrontendTaskAgent` |
| `SUO-226-FE-003` | 配置/执行/失败状态 UI 组件 | frontend | P1 | `state`,`error-ui`,`workflow-status` | `SUO-201-FE-006` | `@FrontendTaskAgent` |
| `SUO-226-SH-001` | Deck/Desk 技术传输合同定义 | shared | P0 | `contract`,`api-spec`,`deck-desk` | `SUO-201-SH-002` | `@BackendTaskAgent` + `@FrontendTaskAgent` |
| `SUO-226-SH-002` | 端到端工作流集成 E2E（Deck→Desk→Agent→审阅） | shared | P0 | `e2e`,`integration`,`deck-desk-agent` | `SUO-226-BE-005`, `SUO-226-FE-001` | `@FrontendTaskAgent` + `@BackendTaskAgent` |

### 2.3 SUO-230 增量新增 Issue（由 DEC-017/DEC-018 设计增量引入）

| Issue ID | 标题 | 类型 | 优先级 | 标签 | 前置依赖 | 分发去向 |
|---|---|---|---|---|---|---|
| `SUO-230-FE-001` | TopNavBar Dream 导航项与 canonical 路由 | frontend | P0 | `navigation`,`router`,`dream-entry`,`delta` | `SUO-201-FE-002` | `@FrontendTaskAgent` |
| `SUO-230-FE-002` | Dream 页面与 ReviewGate 组件 | frontend | P0 | `dream-page`,`review-gate`,`workflow-ui`,`delta` | `SUO-226-FE-001`, `SUO-201-FE-004` | `@FrontendTaskAgent` |
| `SUO-230-BE-001` | 审阅 gate 服务端聚合与防绕过验证 | backend | P0 | `api`,`review-gate`,`idempotency`,`security`,`delta` | `SUO-201-BE-003`, `SUO-226-BE-004` | `@BackendTaskAgent` |
| `SUO-230-SH-001` | 确认幂等与审阅版本校验联调 | shared | P0 | `e2e`,`idempotency`,`review-gate`,`security` | `SUO-230-BE-001`, `SUO-230-FE-002` | `@BackendTaskAgent` + `@FrontendTaskAgent` |
...
- **增量影响**：**高**。原有"Chat 直接触发 Agent"需变为"已选择插件 + Desk 版本引用 → Agent"。输出需关联 `workflow_run_id`。
- 验收条件：保持原基线不变，追加：
  - [ ] Agent 接收执行请求时必须携带已锁定的 `deck_plugin_id` + `deck_plugin_version` + `desk_config_snapshot_id`
...
- 备注：
  - **[CLARIFICATION_NEEDED]** Agent 产出的触发方式：设计稿收敛为"先在 story-workspace 选择 Deck 插件并通过 Desk 校验，再由现有 Chat 指令携带该工作流上下文触发 Claude Agent"
...
- 类型：shared
- 优先级：P0
- **增量影响（SUO-226）**：既有审阅 E2E 保留；需新增 Deck→Desk→Agent→审阅端到端场景（SUO-226-SH-002）
...
- 验收条件：保持原基线不变，追加：
  ...
  - [ ] 错误码类型：`WORKFLOW_SELECTION_REQUIRED` | `PLUGIN_VERSION_UNAVAILABLE` | `DESK_CONFIG_INVALID` | `DESK_UNAVAILABLE` | `WORKFLOW_PERMISSION_DENIED` | `AGENT_EXECUTION_FAILED` | `OUTPUT_CONTRACT_INVALID` | `CONFIG_VERSION_DRIFT`
- 分发去向：`@FrontendTaskAgent` + `@BackendTaskAgent`
- 主责 Agent：`BackendTaskAgent`
...
- 描述：
  根据 SUO-215 设计增量，创建 workflow binding 与 workflow run 的数据模型。Binding 记录用户选择的 Deck 插件与 Desk 配置组合；Run 记录单次执行上下文与状态。保证版本锁定、来源溯源和审计能力。
...
- 验收条件：
  - [ ] `story_workspace_workflow_bindings` 表创建，字段：
    ...
    - `desk_profile_id` (string), `config_version` (string)
...
- 设计决策引用：
  - `DEC-009`：Desk/Deck/Plugin 职责独立
  - `DEC-012`：不存储 Desk 密钥或完整提示词
  - 设计稿 §5.1 选择工作流 / §6.1 DeckPluginManifest

- 备注：
  - **[CLARIFICATION_NEEDED]** Deck 插件 manifest 的物理存储/API 合同尚未确定
  - Clarification owner：`@CEOOrchestrator` 路由 Deck/Desk owner
...
- 标题：Desk 配置预检与就绪状态 API
- 类型：backend
- 优先级：P0
- 标签：`api`,`desk-config`,`preflight`
- 描述：
  实现 Desk 配置预检 API。根据所选 Deck 插件解析其所需的 Desk Agent 提示词与插件配置，校验发布/启用状态、访问权限、配置完整性与版本兼容性。校验通过后返回就绪状态与 `desk_config_snapshot_id`；任一校验失败返回具体错误类别，不启动 Agent。

- 验收条件：
  - [ ] `POST /api/story-workspace/desk-preflight` — 提交 plugin_id + plugin_version，返回预检结果
  - [ ] 预检项：Desk profile 存在性、config_version 有效性、权限校验、schema 兼容性
  - [ ] 通过响应：status=`ready`, desk_profile_id, config_version, desk_config_snapshot_id, 脱敏配置摘要
  - [ ] 失败响应：status=`not_ready`, error_code (`DESK_CONFIG_INVALID`/`DESK_UNAVAILABLE`/`WORKFLOW_PERMISSION_DENIED`), 缺失类别列表（非敏感）
...
- 关联路径：
  - `backend/src/routes/story-workspace/desk-preflight.ts`
  - `backend/src/services/story-workspace/desk-preflight.ts`
...
- 设计决策引用：
  - `DEC-011`：Deck/Desk 前置校验未通过时禁止启动 Claude Agent
  - `DEC-012`：Ink-Dream 只保存版本引用/脱敏摘要
  - 设计稿 §4.2 执行前校验 / §8 错误码定义

- 备注：
  - **[CLARIFICATION_NEEDED]** Desk API、权限模型和密钥存储能力未知
  - Clarification owner：`@CEOOrchestrator` 路由平台 owner
  - 在 Desk 合同确定前，接口可返回 mock 就绪状态，但错误码结构必须按设计稿实现
  - 不得自行编造 Desk API 合同；必须在 Issue 评论区记录待确认项
...
- 描述：
  实现 Workflow Run 的创建、查询和状态管理 API。创作者选择 Deck 插件并通过 Desk 预检后，创建单次 `workflow_run`，固定 `plugin_id + plugin_version + desk_profile_id + config_version`。支持幂等创建（idempotency_key）、状态查询和取消。

- 验收条件：
  - [ ] `POST /api/story-workspace/workflow-runs` — 创建 run，body: { binding_id, input, idempotency_key }
  - [ ] 创建时自动固定版本：plugin_id, plugin_version, desk_profile_id, config_version 从 binding 读取
...
- 备注：
  ...
  - 历史剧本详情必须以只读来源信息展示插件名称/版本、运行 ID 与 Desk 配置快照标识
...
- 类型：backend
- 优先级：P0
- 标签：`agent`,`integration`,`deck-desk`
- 描述：
  适配现有 claude-agent 服务集成，将"Chat 直接触发 Agent"收敛为"已选择插件 + Desk 版本引用 → Agent"。Chat 指令必须携带已校验的 Deck/Desk 运行上下文（`workflow_run_id` + 锁定版本信息）。Agent 使用服务身份向 Desk 解析配置并执行。

- 验收条件：
  - [ ] Chat 触发 Agent 时，请求必须包含 `workflow_run_id` 或引导用户先选择工作流
  - [ ] Agent 接收 `plugin_id`, `plugin_version`, `desk_config_snapshot_id` 及用户输入
  - [ ] Agent 以服务身份向 Desk 解析已授权的配置（不在浏览器端解析）
...
- 描述：
  在 Dashboard 页面顶部实现 Deck 工作流上下文条控件。该控件位于 Main Content Area 的 Dashboard 标题下方，不改变三栏宽度。支持选择已发布可用的 Deck 插件、展示 Desk 配置就绪状态、显示工作流摘要，并提供"在 Deck 编辑器中配置"跳转。

- 验收条件：
  - [ ] `StoryWorkspaceWorkflowContextBar` 组件实现
  - [ ] Deck 插件选择器：下拉展示可用插件名称 + 版本
  - [ ] 选择后展示：插件名称、版本、工作流摘要（步骤数、输入要求）
  - [ ] Desk 配置就绪状态：● 已就绪（绿色）/ ● 未就绪（红色）/ ● 检查中（loading）
  - [ ] "在 Deck 编辑器中配置 ↗" 跳转链接
  - [ ] 未选择状态：占位文案"请选择 Deck 创作工作流"，禁止启动 Agent
  - [ ] 无可用插件状态：空态 + "在 Deck 编辑器中自定义并发布"链接
  - [ ] 插件停用/无权限/版本不可用：保留原选择名称 + 红色原因标记，要求重新选择
  - [ ] Desk 配置缺失：展示非敏感缺失类别 + "前往 Desk 配置"链接
...
- 描述：
  在审阅面板（Review Panel）中增加来源溯源信息展示。用户审阅 Agent 生成内容时，可查看该产出的工作流来源：Deck 插件名称/版本、运行 ID、Desk 配置快照标识、生成时间。Desk 提示词正文和密钥不展示。

- 验收条件：
  - [ ] 审阅面板标题栏下方增加"来源"折叠区域
  - [ ] 展示：Deck 插件名称 + 版本（只读）
  - [ ] 展示：`workflow_run_id`（可点击复制）
  - [ ] 展示：Desk 配置快照标识（只读，非敏感）
  - [ ] 展示：Agent 生成时间
  - [ ] 不展示：Desk 提示词正文、密钥、敏感配置值
...
- 描述：
  实现 Deck/Desk 配置相关状态 UI 组件。包括：配置缺失状态、权限不足状态、执行失败状态、版本漂移状态。这些组件复用现有空态/错误态视觉规范，但针对工作流场景定制文案和恢复动作。

- 验收条件：
  - [ ] `StoryWorkspaceWorkflowUnselectedState`：未选择 Deck 插件，引导选择
  - [ ] `StoryWorkspacePluginUnavailableState`：插件停用/删除/无权限，要求重新选择
  - [ ] `StoryWorkspaceDeskConfigNotReadyState`：Desk 配置缺失，展示缺失类别 + Desk 入口
...
- 标题：Deck/Desk 技术传输合同定义
- 类型：shared
- 优先级：P0
- 标签：`contract`,`api-spec`,`deck-desk`
- 描述：
  定义 story-workspace 与 Deck 插件目录、Desk 配置服务之间的技术传输合同。包括：可用插件查询接口、Desk 预检接口、配置引用格式、权限校验规则、错误码规范。该合同是前后端实现 Deck/Desk 集成的共同依据。
...
- 验收条件：
  ...
  - [ ] 定义 `DeskAgentProfile` 消费合同（desk_profile_id, prompt_version, model_policy, permission_policy, status）
  - [ ] 定义 `DeskPluginRuntimeConfig` 引用合同（desk_profile_id, plugin_id, config_version, schema_version, status）
  - [ ] 定义可用插件查询接口：请求/响应格式、筛选参数、权限头
  - [ ] 定义 Desk 预检接口：请求/响应格式、就绪/错误状态码
  - [ ] 定义错误码规范：`WORKFLOW_SELECTION_REQUIRED`, `PLUGIN_VERSION_UNAVAILABLE`, `DESK_CONFIG_INVALID`, `DESK_UNAVAILABLE`, `WORKFLOW_PERMISSION_DENIED`, `AGENT_EXECUTION_FAILED`, `OUTPUT_CONTRACT_INVALID`, `CONFIG_VERSION_DRIFT`
  - [ ] 定义版本锁定格式：plugin_id + plugin_version + desk_profile_id + config_version
  - [ ] 定义幂等键格式与重试规则
...
- 关联路径：
  - `docs/contracts/story-workspace-deck-desk-api.md`（新建）
  - `shared/types/story-workspace/deck-desk.ts`
...
- 备注：
  - **[CLARIFICATION_NEEDED]** Deck 与 Desk 的正式关系和名称尚未确认
  - **[CLARIFICATION_NEEDED]** Deck manifest 与 Desk 插件配置的物理存储/API 分工尚未确定
...
- 标题：端到端工作流集成 E2E（Deck→Desk→Agent→审阅）
- 类型：shared
- 优先级：P0
- 标签：`e2e`,`integration`,`deck-desk-agent`
- 描述：
  端到端验证完整工作流：创作者在 Deck 编辑器发布插件 → 在 story-workspace 选择插件 → Desk 预检通过 → 锁定版本 → Chat 携带上下文触发 Agent → Agent 按工作流执行 → 产出持久化 → 页面渲染 → 用户审阅确认 → 工作流继续或结束。

- 验收条件：
  - [ ] Deck 插件发布后可被 story-workspace 查询到
  - [ ] 选择插件后 Desk 预检正确返回就绪/错误状态
...
- 备注：
  ...
  - 后端职责：插件目录、Desk 预检、run 创建与管理、Agent 适配、状态流转
...
- `SUO-226-SH-001`（Deck/Desk 技术传输合同定义）是**所有增量 Deck/Desk 集成的阻塞前置**。在该合同未冻结前，下游实现应使用 mock 或最小适配层，不得假设具体 API 形态。
- `SUO-226-SH-001` 依赖 `SUO-201-SH-002`（基础类型定义）。
- `SUO-226-BE-002`（插件目录）和 `SUO-226-BE-003`（Desk 预检）依赖 `SUO-226-SH-001` 的合同定义。
...
- `SUO-201-SH-001`（审阅 E2E）：主责 `FrontendTaskAgent`，协作 `BackendTaskAgent`。
- `SUO-201-SH-002`（命名规范）：主责 `BackendTaskAgent`，协作 `FrontendTaskAgent`。
- `SUO-226-SH-001`（技术传输合同）：主责 `BackendTaskAgent`，协作 `FrontendTaskAgent`。
- `SUO-226-SH-002`（端到端 E2E）：主责 `FrontendTaskAgent`，协作 `BackendTaskAgent`。
- **`SUO-230-SH-001`（确认幂等联调）：主责 `FrontendTaskAgent`，协作 `BackendTaskAgent`。**
...
└── SUO-226-SH-001  Deck/Desk 技术传输合同定义
...
|---|---|---|---|
| `task_201_backend_story-workspace-schema.md` | 需追加 workflow binding/run 表 | 追加 `story_workspace_workflow_bindings`、`story_workspace_workflow_runs` 表定义；stories 表追加 `workflow_run_id` | `@BackendTaskAgent` |
| `task_202_backend_story-workspace-rest-api.md` | 需新增 Deck/Desk/run 端点 | 新增插件目录、Desk 预检、run 创建/查询/取消/重试端点 | `@BackendTaskAgent` |
| `task_205_backend_story-workspace-shared-types.md` | 需扩展类型定义 | 追加 binding、run、manifest、错误码类型 | `@BackendTaskAgent` |
| `task_202e_frontend_dashboard.md` | 需新增工作流上下文条 | 追加 Dashboard Deck 工作流上下文条组件 | `@FrontendTaskAgent` |
| `task_202d_frontend_review-panel.md` | 需追加来源溯源 | 追加审阅面板来源信息展示 | `@FrontendTaskAgent` |
| `task_202g_frontend_e2e-integration.md` | 需扩展 E2E 场景 | 追加 Deck→Desk→Agent→审阅端到端场景 | `@FrontendTaskAgent` |
| `task_202h_frontend_user-documentation.md` | 需补充新功能说明 | 追加插件选择、配置 owner、失败恢复 | `@FrontendTaskAgent` |

### 7.2 需增量更新的 Task 文档（SUO-230）

| Task 文档 | 更新原因 | 更新内容 | 负责 Agent |
|---|---|---|---|
| `task_202b_frontend_sidebar-navigation.md` | 需追加 Dream 导航与 canonical 路由 | 追加 `StoryWorkspaceDreamNavItem` 组件、路由重定向配置 | `@FrontendTaskAgent` |
| `task_202e_frontend_dashboard.md` | 需追加 Dream 页面与 ReviewGate | 追加 `StoryWorkspaceDreamPage` 页面、`StoryWorkspaceReviewGate` 组件 | `@FrontendTaskAgent` |
| `task_202d_frontend_review-panel.md` | 需追加 gate 状态与操作 | 追加 ReviewGate 与 Review Panel 的联动、确认版本校验 | `@FrontendTaskAgent` |
| `task_202_backend_story-workspace-rest-api.md` | 需追加审阅 gate 聚合端点 | 新增审阅状态聚合查询、确认版本校验、继续/结束幂等端点 | `@BackendTaskAgent` |
...
|---|---|---|
| `task_226_backend_workflow-binding-run-schema.md` | Workflow Binding 与 Run 数据模型 | `@BackendTaskAgent` |
| `task_226_backend_deck-plugin-directory-api.md` | Deck 插件目录查询 API | `@BackendTaskAgent` |
| `task_226_backend_desk-preflight-api.md` | Desk 配置预检 API | `@BackendTaskAgent` |
| `task_226_backend_workflow-run-api.md` | Workflow Run 创建与管理 API | `@BackendTaskAgent` |
| `task_226_backend_agent-deck-desk-adapter.md` | Agent 集成适配 | `@BackendTaskAgent` |
| `task_226_frontend_workflow-context-bar.md` | Dashboard 工作流上下文条 | `@FrontendTaskAgent` |
| `task_226_frontend_review-provenance.md` | 审阅面板来源溯源 | `@FrontendTaskAgent` |
| `task_226_frontend_workflow-error-states.md` | 配置/执行/失败状态 UI | `@FrontendTaskAgent` |
| `task_226_shared_deck-desk-contract.md` | Deck/Desk 技术传输合同 | `@BackendTaskAgent` + `@FrontendTaskAgent` |
| `task_226_shared_end-to-end-e2e.md` | 端到端工作流 E2E | `@FrontendTaskAgent` + `@BackendTaskAgent` |

### 7.5 需新建的 Task 文档（SUO-230）

| 建议 Task ID | 内容 | 负责 Agent |
|---|---|---|
| `task_230_frontend_dream-nav-item.md` | TopNavBar Dream 导航项与 canonical 路由 | `@FrontendTaskAgent` |
| `task_230_frontend_dream-page-review-gate.md` | Dream 页面与 ReviewGate 组件 | `@FrontendTaskAgent` |
| `task_230_backend_review-gate-aggregation.md` | 审阅 gate 服务端聚合与防绕过验证 | `@BackendTaskAgent` |
| `task_230_shared_idempotency-e2e.md` | 确认幂等与审阅版本校验联调 | `@FrontendTaskAgent` + `@BackendTaskAgent` |
...
| DEC-011 预检失败禁止启动 | 新增 SUO-226-BE-003 | 新增 Desk 预检 API task | 新增预检失败场景测试 | 预检 API 实现 |
...
| Deck/Desk 集成 Delta | 新增 10 条增量 Issue | 新增 10+ task | 新增 2-3 waves | 新增实现范围 |
...
- `stage_story-workspace.md`（现有 Stage）基于 SUO-199 设计稿制定，**不含 Deck/Desk 依赖和准入门**。
- 本次设计增量（SUO-214/SUO-215）引入的 Deck/Desk/Agent 端到端集成**必须在新的增量 Stage 中验证**。
- **SUO-230 增量**引入的 Dream 导航与审阅 Gate**必须在另一独立增量 Stage 中验证**。
- **建议**：
  - 保留现有 `stage_story-workspace.md` 作为基线 Stage
  - 新建 `stage_story-workspace-deck-desk-integration.md` 作为 Deck/Desk 增量 Stage
  - 新建 `stage_story-workspace-dream-gate.md` 作为 Dream 导航与 Gate 增量 Stage
  - Deck/Desk 增量 Stage 置于配置/类型合同完成之后（SUO-226-SH-001 冻结后）
...
### 9.1 [CLARIFICATION_NEEDED] Deck 与 Desk 的正式关系和名称

- **歧义点**：Deck 与 Desk 是否为两个独立产品/服务、同一产品的两个模块，或评论中的拼写差异，当前无法从现有上下文收敛。
- **可能解释 A**：两个独立产品/服务
- **可能解释 B**：同一产品的两个模块
- **默认采用解释**：两个逻辑边界；即使最终由同一服务承载，也必须保留"插件定义"和"Agent 运行配置"两类独立所有权
- **需要确认方**：`@CEOOrchestrator` 路由产品 owner
- **是否阻塞 task 阶段**：**否**（可采用默认假设继续，但合同冻结前使用 mock/适配层）
- **风险**：错误合并会造成所有权和敏感数据边界混乱
- **Clarification owner / action**：`@CEOOrchestrator` 确认正式名称、服务边界及 API owner，并在下游增量 Issue/Task 中引用

### 9.2 [CLARIFICATION_NEEDED] Deck manifest 与 Desk 插件配置的物理存储/API 分工

- **歧义点**：Deck 插件 manifest 和 Desk 配置的具体物理存储位置、API owner、版本管理方式尚未确定
- **可能解释 A**：Deck 保存 manifest；Desk 保存运行配置和密钥引用
- **可能解释 B**：统一存储，逻辑分层
- **默认采用解释**：Deck 保存 manifest；Desk 保存运行配置和密钥引用
- **需要确认方**：`@CEOOrchestrator` 路由 Deck/Desk owner
- **是否阻塞 task 阶段**：**否**（接口草案按默认假设定义，标记 `[DRAFT]`）
- **风险**：重复配置、版本不一致
- **Clarification owner / action**：`@CEOOrchestrator` 提供版本和读取合同

### 9.3 [CLARIFICATION_NEEDED] Desk 配置停用时已运行任务的策略

- **歧义点**：Desk profile 被停用后，已开始 run 的处理策略
...
### 9.5 [BLOCKED] SUO-226-SH-001（Deck/Desk 技术传输合同）在 API 合同冻结前无法完全实现

- **阻塞原因**：Deck/Desk API 合同尚未由平台 owner 确认
- **影响范围**：SUO-226-BE-002（插件目录）、SUO-226-BE-003（Desk 预检）、SUO-226-BE-005（Agent 适配）
- **当前责任 Agent**：IssueDispatcher（已标记）
- **需要唤醒的 Agent**：`@CEOOrchestrator` 路由产品/平台 owner 确认 Deck/Desk 合同
- **建议处理方式**：
