...
// [Sync] 2026-09-06: prove direct and reconnect paths retain trusted MCP App result identity.
...
test('reconnect replay preserves MCP App result identity without changing tool output', () => {
  const projection = {
    version: 1,
    ...
    input: { city: 'Shanghai' },
    workspaceScope: null,
    resourceUri: 'ui://weather/current.html',
  ...
  const messages = [
    {
      type: 'tool-input-available',
      toolCallId: 'call-app',
      toolName: 'mcp__official-basic__get-weather',
  ...
  expect(messages[0].parts[0]).toMatchObject({
    ...
    output: projection.result,
    toolMetadata: { mcpAppResult: projection },
  });
});
...
test('primary Chat transport emits MCP App identity before the matching output', async () => {
  class TestTransport extends ClaudeAgentChatTransport {
    convert(stream: ReadableStream<Uint8Array>) {
      return this.processResponseStream(stream);
    }
  ...
  const raw = [
    { type: 'tool-input-start', toolCallId: 'call-app', toolName: 'mcp__official-basic__get-weather' },
    { type: 'tool-input-available', toolCallId: 'call-app', toolName: 'mcp__official-basic__get-weather', input: projection.input },
    { type: 'tool-output-available', toolCallId: 'call-app', output: projection.result, isError: false, mcpAppResult: projection },
  ...
  const identityIndex = chunks.findIndex((chunk) => (
    chunk.type === 'tool-input-available'
    && (chunk.toolMetadata as { mcpAppResult?: unknown } | undefined)?.mcpAppResult !== undefined
  ));
  const outputIndex = chunks.findIndex((chunk) => chunk.type === 'tool-output-available');
  expect(identityIndex).toBeGreaterThan(-1);
  expect(identityIndex).toBeLessThan(outputIndex);
  expect(chunks[outputIndex]?.output).toEqual(projection.result);
});
