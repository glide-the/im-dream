...
Public user turns now reserve message/title through Admin's atomic confirmation guard command using raw Python JSON. A server-only Thread/Run persistence grant renews under Factory ownership and survives SSE disconnect; unknown writes keep their original receipt ID. Public Agent Thread reads, SDK-native session ID updates, recent writing Session context, and complete/cancelled/error assistant messages use that bound grant. User/session/assistant writes share original-receipt recovery, and only the latest confirmed Session update is reused. Internal dispatcher assistant writes, Session tools, background Session consumers, Gateway credentials and other database domains remain migration work.

Public writing Session save/get/batch/list/range/aggregate/delete now use six typed Admin operations with explicit request OAuth. The original metadata/full-state responses, precise timestamps, timezone day keys and prose metrics remain; edit events publish only after confirmed writes. Shared closed Editor state DTOs omit absent optional fields. The same `session.list` contract accepts the exact Thread-bound `server-persistence` grant only for Agent prompt context: Service requests UTC today and the prior two days on prompt build or Settings prompt rebuild, then ContextBuilder renders the strict projection without database access. Admin or contract failures stop before Runtime startup. Session tools and Reflections background reads remain separate authorization work.
