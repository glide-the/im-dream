        ...
        const renderPart = (partIndex: number, partKind: 'normal' | 'process' | 'outside-app' | 'final') => {
              ...
              if (isToolUIPart(part)) {
                ...
                if (needsEditorWriteApproval) {
                  return (
                    <div key={partKey}>
                      <ToolMessagePart part={toolPart} threadId={threadId} isLast={isLastMessage} isLoading={isLoading} addToolResult={addToolResult} sendUserMessage={sendUserMessage} onEditorWriteConfirmed={onEditorWriteConfirmed} settledToolCallIds={settledToolCallIds} onConfirmationSettled={onToolConfirmationSettled} />
                    </div>
                  );
                }
                ...
                return (
                  <div key={partKey} style={{ paddingLeft: '0.85rem', borderLeft: `2px solid ${pendingConfirmationKind ? '#f59e0b' : 'var(--color-action-link)'}` }}>
                    ...
                    {effectiveExpanded ? <div style={{ marginTop: '0.6rem' }}><ToolMessagePart part={toolPart} threadId={threadId} isLast={isLastMessage} isLoading={isLoading} addToolResult={addToolResult} sendUserMessage={sendUserMessage} settledToolCallIds={settledToolCallIds} onConfirmationSettled={onToolConfirmationSettled} /></div> : null}
                  </div>
                );
              }
