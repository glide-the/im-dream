...
| AutoDL SSH 发布 | [`autodl-ssh/deploy.sh`](autodl-ssh/deploy.sh) | SSH/rsync、直接本机 Python/Node 与 screen；Vite Preview 监听 `127.0.0.1:6006` 并代理 FastAPI `127.0.0.1:8765`，Admin upstream 为 `127.0.0.1:6008`，Claude plugin artifacts 持久化到数据盘，不使用 Docker/nginx |
