...
> **[Sync] 2026-09-06**: 资源链接保留远程/本地连续结构；Claude MCP 新增 Server 改用可访问弹窗，连接详情把完整 App 设置并入“使用策略”并在修改后自动保存。
...
- 弹窗使用明确标题和关闭标签，打开后聚焦“服务名称”，支持 Tab 焦点约束、Escape / 遮罩关闭和关闭后焦点恢复。桌面端使用适合连接配置的宽面板，把名称/传输归为“连接信息”双列组，把 URL/profile 归为独立全宽组；说明与字段位于可滚动正文，取消/添加操作固定在独立底部区。窄屏改为单列，短视口只滚动正文、底部操作始终可见且按钮占满可用宽度，不产生横向滚动。
- transport 选项、认证由后端 discovery 判定、客户端校验、保存、错误保留和成功后刷新列表的既有业务规则不变。
- Server 详情只保留一个“使用策略”区域：完整包含“在聊天中使用 App”“低风险工具调用”“向聊天发送消息”，开关只表达用户 `desired`。
- “使用策略”标题下不显示解释 subtitle，区域内不显示 App 总状态、低风险调用或消息交互的 availability 徽标，底部也不显示 `default`、`desired`、`revision`、实际原因或自动保存状态汇总；这些值仍保留在 API/CAS 与 Runtime 组合合同中，不因 UI 精简而删除。
- 页面不得再出现独立“App 设置”标题、“保存 App 设置”或“保存使用策略”按钮。任一 App 选择修改后自动保存；保存中与成功不增加技术性提示，失败时在区域内保留当前选择并显示可恢复错误，不得只删除按钮而失去保存入口。
- 自动保存必须串行使用现有 revision/CAS 接口。保存期间继续发生的修改进入待保存队列；revision conflict 时读取最新 settings，只把本地未确认字段合并到最新 `desired` 后重试，未修改字段不得覆盖其他客户端的并发选择。Server 配置保存、OAuth、inventory 与 Chat 工具审批仍是独立业务边界。
- `desired=true` 不等于 Runtime 已经实际生效；页面不展示 availability 提示，但 Runtime 仍必须按服务端上限组合 `effective`，并在服务端恢复后沿用同一 revision，无需用户重复保存。
