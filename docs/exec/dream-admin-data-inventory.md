...
| 公开写作Session CRUD/summary | [sessions router](../../backend/routers/sessions.py)/[session_data](../../backend/services/admin_data/session_data.py)：session.save/get/batch/list/text-list/delete | 路由无Dream DB；原metadata/state/microsecond/timezone/metrics/confirmed edit events；`session.list`另接受精确Thread-bound server-persistence recent-context read | Session工具与Reflections后台仍有原DB入口；其他五项Session operation不接受该purpose |
| 公开Agent Thread/SDK/recent Session | [Service](../../backend/claude_agent/service.py)/[turn_persistence](../../backend/services/admin_data/turn_persistence.py)：chat-thread.get/update-session/thread-system-config.get/session.list | resume、Session回写、fresh SystemConfig与prompt rebuild的UTC三日Session投影使用exact server grant；user/session/assistant共享unknown屏障，三项读取参与owner drain；ContextBuilder无DB | 缺owner内部dispatcher在配置边界fail closed；Session tools、Reflections background及其余内部PG/MCP独立迁移 |
