...
Resources 初始加载并行请求 capability/list；未启用时只展示 capability 错误。list card 展示数据库状态，不把 stale 等同 disconnected，也不触发远端 discovery。资源链接页继续使用既有“远程资源链接 / 本地资源链接”连续结构，不增加知识库、MCP、本地资源二级分区。新增 Server 的名称、transport、URL/profile 表单只在用户点击“添加 MCP 服务”后进入共享可访问弹窗；桌面宽面板把名称/transport 与 URL/profile 分组，正文独立滚动、底部操作区始终可见，窄屏退化为单列且不横向溢出。成功保存才关闭并清空表单，校验或 API 失败保留输入并在固定底部区反馈；Escape 关闭后恢复触发按钮焦点。
