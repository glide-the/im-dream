...
### 允许修改范围
- 允许创建 `docs/task/story-workspace-user-guide.md`（或等效路径的用户文档）
...
### 明确排除项
- **复杂画布**：文档中不涉及画布操作指南、图形编辑教程、SVG 绘制说明等
- **视频**：文档中不包含视频教程、视频操作指南、视频审阅流程说明等
- **移动端**：文档明确说明 Story Workspace 仅支持桌面端（≥1280px），不包含移动端使用指南、移动端适配说明
- **用户手动创建内容**：文档中的工作流说明明确为「Agent 生成 → 用户审阅确认」，FAQ 中说明「如何触发 Agent 生成剧本？」而非「如何手动创建剧本？」。文档不描述用户手动创建内容的操作步骤
- **技术架构细节**：本文档为使用文档，不涉及系统架构图、数据库设计、API 详细实现等技术架构内容
- **开发指南**：不涉及组件开发规范、代码贡献指南、本地开发环境搭建等面向开发者的文档
- **运维文档**：不涉及部署指南、监控告警、日志查看等运维相关文档
