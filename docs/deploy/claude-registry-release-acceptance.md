...
<!-- [同步] 2026-09-13：把本页明确标为最近一次已完成的历史回执；当前源码合同 0.2.145 × 0.1.7 尚未发布，不能复用本页证据。 -->
...
当前 `develop` 源码合同。当前源码要求 SDK `0.2.145` × Runtime `0.1.7`；截至
2026-09-13，npm `latest` 仍为 `0.1.4`，因此必须等 `0.1.7` 同字节发布后重新
