...
<!-- [同步] 2026-09-13：当前源码合同升级为 SDK 0.2.145 × 未发布 Runtime 0.1.7 package-root cli.js；0.1.4 证据降为历史。 -->

# 项目架构设计说明

> 前端代码证据：`54f3bbe539b086035bce5900618bf3d7ae5b9372`（2026-09-06）。Runtime registry 查询日期：2026-09-13。
>
> 当前状态：Dream 源码使用自有 Python SDK `0.2.145`，默认 Runtime 合同为未发布的 clean-room `0.1.7` package-root selector；官方 Claude CLI 只作为显式绝对路径回滚。本文不据源码推断正在运行的服务版本。
>
> 发布状态：Dream 使用正式 PyPI 精确版本和 wheel/sdist SHA-256 锁固定 SDK `0.2.145`。npm registry 在 2026-09-13 的 `latest` 仍为 `0.1.4`；`0.1.7` 的 production、redistribution、publication 与四 target gate 全部关闭，旧 `0.1.5` acceptance 不复用。
>
...
| 对象 | 当前版本或身份 | 当前适用性 |
| --- | --- | --- |
| Dream 项目元数据 | backend `0.1.1`、frontend `0.0.1`；API schema `2.0.0` 不变 | 现有项目版本序列的 patch 更新，不代表部署或 API 变更 |
...
| clean-room Runtime package | source contract `@glide-the/ink-claude-code-dream@0.1.7` | `package/cli.js` selector；对 Dream 输出 `2.1.241 (Claude Code)`；manifest 配对 SDK `0.2.145`；尚未发布 |
| Runtime 历史发布 | `0.1.4` / `main@0ebafe95db22101cf77db2c27e73b561d3af37a6` | qualification `33306855166` 与 publish `33306940462` 仅为旧制品证据 |
| Runtime 实现输入 | `package/` selector 源 + `src/cleanroom/` MIT Runtime + 兼容许可证依赖 | 禁止输入 restored/vendor source、source map 与用户数据；实际新 tree/digest 由 0.1.7 qualification 生成 |
| Runtime 研究源码快照 | 原始 `restored-src/src`：1,902 文件、35 模块目录；subtree `7640f58e…ebe30` | Anthropic 版权、非 MIT、精确字节不改名；只保存在 Runtime 仓库，排除于上述 clean-room 构建/制品图 |
...
| registry 验收 Runtime（历史） | 0.1.4 source tree `266362ac…95de81`；darwin-arm64 executable `969f9193…980dd` | 只证明旧发布；0.1.7 必须产生新 fresh-install 回执 |
| Docker 默认 Runtime source pin | npm `@glide-the/ink-claude-code-dream==0.1.7` | 解析 package-root `cli.js` 与相邻 manifest；发布前精确安装 fail closed |
...
版本结论：`2.1.241` 是 Dream 面向 Runtime 的 CLI/协议兼容标识，不表示官方全产品等价。当前候选的实现依据是独立 clean-room 代码、manifest、SBOM 和协议测试；新同 SHA 真实业务/发布回执仍缺失，不能复用旧证据。完整研究源码结构的恢复见[接入说明](../deploy/runtime-0.1.7-source-structure-integration.md)。
...
未发布 `0.1.7`；实际服务是否已继承某个 PATH 仍以该受控进程启动时的 resolver/manifest 回执为准，不能从磁盘安装树推断：
...
```text
@glide-the/ink-claude-code-dream@0.1.7
  ├─ cli.js：package-root selector
  └─ 根据 process.platform/process.arch 选择可选依赖

@glide-the/ink-claude-code-dream-darwin-arm64@0.1.7
@glide-the/ink-claude-code-dream-darwin-x64@0.1.7
@glide-the/ink-claude-code-dream-linux-arm64@0.1.7
@glide-the/ink-claude-code-dream-linux-x64@0.1.7
```
