    ...
    return (
      <div
        style={{
          ...
          marginRight: isUserMessage ? undefined : 'auto',
          background: 'var(--color-bg-paper)',
        }}
      >
        <img src={fileUrl} alt={part.filename || 'Uploaded image'} loading="lazy" style={{ display: 'block', width: '100%', height: 'auto' }} />
        {part.filename && <div style={{ padding: '0.625rem 0.875rem', fontSize: '0.875rem', color: 'var(--color-text-secondary)' }}>{part.filename}</div>}
      </div>
    );
  }
  ...
  return (
    <div
      style={{
        maxWidth: '28rem',
        borderRadius: '18px',
        border: `1px solid ${isUserMessage ? 'rgba(255,255,255,0.25)' : 'var(--color-border-paper)'}`,
        padding: '1rem',
        boxShadow: '0 8px 20px var(--color-shadow-soft)',
        background: isUserMessage ? 'var(--color-action-link)' : 'var(--color-bg-paper)',
        color: isUserMessage ? '#fff' : 'var(--color-text-primary)',
        marginLeft: isUserMessage ? 'auto' : undefined,
        marginRight: isUserMessage ? undefined : 'auto',
      }}
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
        <div
          style={{
            ...
            borderRadius: '14px',
            padding: '0.75rem',
            background: isUserMessage ? 'rgba(255,255,255,0.14)' : 'var(--color-bg-surface)',
          }}
        >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '0.5rem', marginTop: '0.4rem', fontSize: '0.75rem', color: isUserMessage ? 'rgba(255,255,255,0.8)' : 'var(--color-text-muted)' }}>
            <span
              style={{
                ...
                borderRadius: '999px',
                border: `1px solid ${isUserMessage ? 'rgba(255,255,255,0.28)' : 'var(--color-border-paper)'}`,
        ...
        {fileUrl ? (
          <a
            ...
            style={{
              ...
              height: '2rem',
              borderRadius: '999px',
              color: isUserMessage ? 'rgba(255,255,255,0.84)' : 'var(--color-text-secondary)',
              textDecoration: 'none',
            }}
          >
