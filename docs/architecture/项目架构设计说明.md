...
<!-- [同步] 2026-08-24：记录 MIT clean-room Runtime main@7c34e6cd、公开 npm 五包、SDK main 固定提交、无 source map 和真实 Comfy MCP 两轮 resume 证据。 -->
...
>
> 发布状态：SDK 已合入 `main`，Dream 当前固定 SDK `main` 的不可变提交；Runtime PR #5/#6 已合并，当前为 `main@7c34e6cd93a35cc35d84e365382db1227db3e861`，五个 `0.1.0` npm 包均已公开并通过 registry fresh install。SDK PyPI 尚未发布。
>
...
| 自有 SDK distribution | `ink-claude-dream-agent-sdk==0.2.143` | Dream `pyproject.toml`、`uv.lock`、`requirements.txt` 固定提交 `6164bd91e43bbf610ec40b4500edec18a97ce665` |
...
| SDK 仓库 | `main@6164bd91e43bbf610ec40b4500edec18a97ce665` | wheel/sdist 流程已合并；PyPI/TestPyPI 当前仍未发布 |
...
sequenceDiagram
    ...
    US->>SR: 固定 commit/tree 并验证 src 空差异
...
| SDK 产物 | wheel 151,684 B，SHA `e64ea7bf…a27bcd1`；sdist 372,249 B，SHA `d669d0be…c0469a`；均无 `.map` |
| Dream SDK 接入 | `uv lock`、`uv export`、`uv sync --frozen --inexact` exit 0；distribution 唯一且提交为 `6164bd91…` |
...
- SDK PyPI/TestPyPI 尚未发布；发布需要 SDK 项目的独立授权和 Trusted Publisher 门，不在 Dream 项目执行。
