...
<!-- [同步] 2026-08-28：Dream 源码/Docker 精确采用公开 Runtime 0.1.3；记录 opaque 模型输出能力、same-SHA workflow、registry fresh install 和真实 Gateway/Admin 验收。 -->
...
>
> 发布状态：SDK `0.2.144` 已通过 Trusted Publisher 同时发布到 TestPyPI 与正式 PyPI；Dream 使用正式 PyPI 精确版本和 wheel/sdist SHA-256 锁。Runtime `main@9339c9a0ff60e1b2cd6d5a23c8e795aeffff91f9` 的五个 `0.1.3` npm 包均已公开并通过 provider-free registry fresh install；最小五包权限 npm token 与 GitHub `npm` Environment Secret 按用户要求保留。
>
...
| clean-room Runtime package | `@glide-the/ink-claude-code-dream@0.1.3` | npm 公开 selector；对 Dream 输出 `2.1.241 (Claude Code)` 兼容标识；manifest 配对 SDK `0.2.144` |
| Runtime 仓库 | release `main@9339c9a0ff60e1b2cd6d5a23c8e795aeffff91f9` | qualification run `33157330350` 与 publish run `33157476036` 均成功 |
| Runtime 实现输入 | `src/cleanroom/` 48 个文件、291,163 字节；tree SHA-256 `c8d0a7ecff51833f82d5c4c9fd1cffd7cb4a8bf8f0a8227ce85e701779c87cb9` | 仓库自有 MIT 代码；禁止输入扫描不含 restored/vendor 代码 |
...
| registry 验收 Runtime | v2 source tree `c8d0a7ec…c87cb9`；darwin-arm64 executable `9b109064…9d1d4` | fresh install 只选择当前平台包；两个 CLI alias、manifest 配对、exact native digest 与零 `.map` 通过 |
...
```text
@glide-the/ink-claude-code-dream@0.1.3
  └─ selector：根据 process.platform/process.arch 选择可选依赖

@glide-the/ink-claude-code-dream-darwin-arm64@0.1.3
@glide-the/ink-claude-code-dream-darwin-x64@0.1.3
@glide-the/ink-claude-code-dream-linux-arm64@0.1.3
@glide-the/ink-claude-code-dream-linux-x64@0.1.3
```
...
五个包已按四个平台包优先、selector 最后的顺序发布到 npm。公共 packument 均返回 `0.1.3`、MIT 和对应 `os/cpu`；全新目录安装只选择 Darwin ARM64 平台包，`ink-claude-code-dream --version`、`claude --version`、manifest 配对和零 `.map` 检查全部退出 0。表中 SHA-256 来自 registry 匿名 `npm pack` 的验收输出，不冒充 qualification artifact 的原始 tgz；publish job 只下载、复验并发布同 SHA qualification 制品，没有重新构建。
...
| Runtime 测试 | release CI `118 total, 114 passed, 4 conditional external-fixture skips, 0 fail`；lint exit 0 |
...
| npm registry fresh install | `0.1.3` selector + darwin-arm64；两个 CLI alias `--version` 为 `2.1.241 (Claude Code)`；manifest 配对 SDK `0.2.144`；无 `.map`；provider-free wheel/sdist 双路径均通过 |
