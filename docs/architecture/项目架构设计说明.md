...
<!-- [Sync] 2026-09-16: five public Deck Plugin binding routes consume Admin Registry122-126. -->
...
Registry122-126 已关闭五个公开 Deck Plugin binding 数据路径：Dream 使用严格 Pydantic DTO 调用 Admin current/history/options/validate/save，Admin 用 typed Drizzle Repository 执行 owner、compatibility、CAS、历史和持久化。Dream 保留公开页面、路由响应、Runtime 与共享文件系统；`agent-type` 的本地 materialization 与其数据库依赖仍是下一阶段。
