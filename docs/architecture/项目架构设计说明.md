...
flowchart LR
    ...
    API -->|"strict business DTO APIs"| ADMIN
...
| Dream API | Admin 认证与数据接口消费、Thread/Run/Workspace 编排、managed MCP、SDK options、Agent Runtime、SSE 与共享文件操作 | 不连接 PostgreSQL、不管理共享 Schema，不实现第二套 Agent/Runtime 协议 |
| Admin | Better Auth、业务 DTO/Service/typed Drizzle Repository、Drizzle Schema、PostgreSQL、模型、订阅、计费、Gateway | Dream 不复制身份、事务、权限过滤、账本和模型路由 |
