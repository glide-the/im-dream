...
<!-- [同步] 2026-09-13：当前源码合同升级为 SDK 0.2.145 × 未发布 Runtime 0.1.9 package-root cli.js；0.1.4 证据降为历史。 -->

# 项目架构设计说明

> 前端代码证据：`54f3bbe539b086035bce5900618bf3d7ae5b9372`（2026-09-06）。Runtime registry 查询日期：2026-09-13。
>
> 当前状态：Dream 源码使用自有 Python SDK `0.2.145`，默认 Runtime 合同为未发布的原始模块统一实现 `0.1.9` package-root selector；官方 Claude CLI 只作为显式绝对路径回滚。本文不据源码推断正在运行的服务版本。
>
> 发布状态：SDK `0.2.145` 的正式 PyPI 精确版本/wheel/sdist SHA-256 锁不变。Runtime `0.1.9` 获用户确认的来源授权，本机 full qualification 通过；四 target/公开 npm/运行采用须新同 SHA 回执，状态见[发布接入说明](../deploy/runtime-0.1.9-release-and-local-dream-adoption.md)。旧 `0.1.5` acceptance 不复用。
>
...
| 原始模块 Runtime package | source contract `@glide-the/ink-claude-code-dream@0.1.9` | `package/cli.js` selector；对 Dream 输出 `2.1.241 (Claude Code)`；manifest 配对 SDK `0.2.145`；尚未发布 |
...
| registry 验收 Runtime（历史） | 0.1.4 source tree `266362ac…95de81`；darwin-arm64 executable `969f9193…980dd` | 只证明旧发布；0.1.9 必须产生新 fresh-install 回执 |
| Docker 默认 Runtime source pin | npm `@glide-the/ink-claude-code-dream==0.1.9` | 解析 package-root `cli.js` 与相邻 manifest；发布前精确安装 fail closed |
...
版本结论：`2.1.241` 是 Dream 面向 Runtime 的 CLI/协议兼容标识，不表示官方全产品等价。当前候选实现依据是实际编译的原始 src 模块、源码绑定的 headless/MCP 变换、manifest 和协议测试；新同 SHA 真实业务/发布回执仍缺失，不能复用旧 MIT 实现的证据。完整目录与模块统一见[接入说明](../deploy/runtime-0.1.9-release-and-local-dream-adoption.md)。
...
未发布 `0.1.9`；实际服务是否已继承某个 PATH 仍以该受控进程启动时的 resolver/manifest 回执为准，不能从磁盘安装树推断：
