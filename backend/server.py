...
@app.get("/api/storage")
async def get_storage_config(credentials: HTTPAuthorizationCredentials = Depends(http_bearer)):
    """
    GET /api/storage
...
async def upload_file(
    ...
    if not check["is_valid"]:
        raise HTTPException(
            status_code=500,
            detail={"error": check.get("error"), "solution": check.get("solution"), "storageDriver": storage_driver},
        )
